...
APP_VERSION = "0.6.19"
APP_TIMEZONE_NAME = os.getenv("M17_TIMEZONE", "Asia/Bangkok").strip() or "Asia/Bangkok"
try:
    APP_TIMEZONE = ZoneInfo(APP_TIMEZONE_NAME)
except Exception:
    APP_TIMEZONE_NAME = "Asia/Bangkok"
    APP_TIMEZONE = ZoneInfo(APP_TIMEZONE_NAME)

def app_now() -> datetime:
    """Current application time. Workout-day access is evaluated in the configured M17 timezone."""
...
class WorkoutLog(BaseModel):
    workout_date: str = Field(default_factory=lambda: app_today().isoformat())
...
class TrainingCycleCreate(BaseModel):
    ...
    sport: str = Field(default="crossfit", pattern="^(crossfit|running|triathlon|strength|fat_loss|muscle_gain|figure|general)$")
...
class TrainingCycleEdit(BaseModel):
    ...
    sport: Optional[str] = Field(default=None, pattern="^(crossfit|running|triathlon|strength|fat_loss|muscle_gain|figure|general)$")
...
class ExerciseLibraryImport(BaseModel):
    text: str = Field(min_length=1, max_length=500000)
    overwrite_existing: bool = True


class ExerciseVisualImage(BaseModel):
    filename: str = Field(min_length=1, max_length=180)
    mime_type: str = Field(default="image/webp", max_length=80)
    data_base64: str = Field(min_length=1, max_length=10_000_000)


class ExerciseVisualUpload(BaseModel):
    images: list[ExerciseVisualImage] = Field(min_length=1)
    replace_existing: bool = True


class ExerciseVisualWorkflowUpdate(BaseModel):
    status: str = Field(pattern="^(NO_VISUAL|PROMPT_READY|CREATED|REVIEW|FIX_REQUIRED|APPROVED)$")
    note: Optional[str] = Field(default=None, max_length=1200)


class ExerciseVisualWorkflowBatchUpdate(BaseModel):
    exercise_codes: list[str] = Field(min_length=1, max_length=2000)
    status: str = Field(pattern="^(NO_VISUAL|PROMPT_READY|CREATED|REVIEW|FIX_REQUIRED|APPROVED)$")
    only_if_status: Optional[str] = Field(default=None, pattern="^(NO_VISUAL|PROMPT_READY|CREATED|REVIEW|FIX_REQUIRED|APPROVED)$")


class ExerciseManualLink(BaseModel):
    source_type: str = Field(pattern="^(cycle|assignment)$")
    source_id: int = Field(ge=1)
    line_no: int = Field(ge=1)
    exercise_code: str = Field(min_length=1, max_length=80)


class ExerciseLibraryEdit(BaseModel):
    name_ru: Optional[str] = Field(default=None, min_length=1, max_length=180)
    family: Optional[str] = Field(default=None, max_length=100)
    movement_pattern: Optional[str] = Field(default=None, max_length=180)
    primary_muscles: Optional[str] = Field(default=None, max_length=500)
    secondary_muscles: Optional[str] = Field(default=None, max_length=500)
    equipment: Optional[str] = Field(default=None, max_length=500)
    difficulty: Optional[str] = Field(default=None, max_length=300)
    program_tags: Optional[str] = Field(default=None, max_length=1000)
    goals: Optional[str] = Field(default=None, max_length=1000)
    load_method: Optional[str] = Field(default=None, max_length=300)
    one_rm_allowed: Optional[str] = Field(default=None, max_length=30)
    rpe_range: Optional[str] = Field(default=None, max_length=80)
    rep_range: Optional[str] = Field(default=None, max_length=100)
    tempo: Optional[str] = Field(default=None, max_length=200)
    rest_range: Optional[str] = Field(default=None, max_length=120)
    technique: Optional[str] = Field(default=None, max_length=4000)
    common_errors: Optional[str] = Field(default=None, max_length=4000)
    regression: Optional[str] = Field(default=None, max_length=1000)
    progression: Optional[str] = Field(default=None, max_length=1000)
    client_tip: Optional[str] = Field(default=None, max_length=1200)
    coach_review_flags: Optional[str] = Field(default=None, max_length=1200)
    source_internal: Optional[str] = Field(default=None, max_length=500)
    visual_id: Optional[str] = Field(default=None, max_length=500)
    client_show_source: Optional[bool] = None
    ...
    with db_conn() as conn:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS exercise_library (
                id {id_type} PRIMARY KEY{id_suffix}, exercise_code TEXT NOT NULL, owner_telegram_id TEXT NOT NULL,
                name_ru TEXT NOT NULL, family TEXT, movement_pattern TEXT, primary_muscles TEXT, secondary_muscles TEXT,
                equipment TEXT, difficulty TEXT, program_tags TEXT, goals TEXT, load_method TEXT, one_rm_allowed TEXT,
                rpe_range TEXT, rep_range TEXT, tempo TEXT, rest_range TEXT, technique TEXT, common_errors TEXT,
                regression TEXT, progression TEXT, client_tip TEXT, coach_review_flags TEXT, source_internal TEXT,
                visual_id TEXT, client_show_source INTEGER NOT NULL DEFAULT 0, active INTEGER NOT NULL DEFAULT 1,
                created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
                UNIQUE(owner_telegram_id, exercise_code)
            )
            """
        )
        conn.execute(
            f"""
            CREATE TABLE IF NOT EXISTS exercise_visuals (
                id {id_type} PRIMARY KEY{id_suffix}, owner_telegram_id TEXT NOT NULL, exercise_code TEXT NOT NULL,
                sort_order INTEGER NOT NULL DEFAULT 1, filename TEXT NOT NULL, mime_type TEXT NOT NULL,
                image_data TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
                UNIQUE(owner_telegram_id, exercise_code, sort_order)
            )
            """
        )
        conn.execute(
            f"""
            CREATE TABLE IF NOT EXISTS exercise_visual_workflow (
                id {id_type} PRIMARY KEY{id_suffix}, owner_telegram_id TEXT NOT NULL, exercise_code TEXT NOT NULL,
                status TEXT NOT NULL DEFAULT 'NO_VISUAL', note TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
                UNIQUE(owner_telegram_id, exercise_code)
            )
            """
        )
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS workout_exercise_links (
                id {id_type} PRIMARY KEY{id_suffix}, owner_telegram_id TEXT NOT NULL,
                source_type TEXT NOT NULL, source_id BIGINT NOT NULL, line_no INTEGER NOT NULL,
                source_line TEXT NOT NULL, exercise_code TEXT NOT NULL,
                match_type TEXT NOT NULL DEFAULT 'AUTO', confidence REAL NOT NULL DEFAULT 1.0,
                created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
                UNIQUE(owner_telegram_id, source_type, source_id, line_no, exercise_code)
            )
            """
        )
        # v0.6.9: source workout IDs are BIGSERIAL on PostgreSQL. Older v0.6.8
        # created source_id as INTEGER, which can make manual linking fragile on an
        # existing PostgreSQL database. Repair the column type at startup.
        if USE_POSTGRES:
            try:
                conn.execute("ALTER TABLE workout_exercise_links ALTER COLUMN source_id TYPE BIGINT USING source_id::BIGINT")
            except Exception as exc:
                # If the backend/DB already exposes BIGINT (or a provider rejects a no-op
                # ALTER), continue; CREATE TABLE above still guarantees the table exists.
                try:
                    conn.conn.rollback()
                except Exception:
                    pass
                # Re-open the surrounding transaction by executing a harmless query.
                conn.execute("SELECT 1")
        migrate_legacy_figure_structure(conn)
        conn.commit()


LEGACY_FIGURE_GOALS = {"fat_loss", "muscle_gain"}


def migrate_legacy_figure_structure(conn):
    """Объединяет старые цели похудение/набор мышц в единый блок figure."""
    now = utc_now()
    rows = conn.execute("SELECT telegram_user_id,primary_goal,goals_json,goal_details_json FROM clients").fetchall()
    for row in rows:
        ...
        try:
            ...
            goals = []
        try:
            details = json.loads(row["goal_details_json"] or "{}")
        except Exception:
            details = {}
        legacy = [g for g in goals if str(g.get("goal")) in LEGACY_FIGURE_GOALS]
        has_legacy_primary = primary in LEGACY_FIGURE_GOALS
        if not legacy and not has_legacy_primary:
            continue
        legacy_names = {str(g.get("goal")) for g in legacy}
        if has_legacy_primary:
            legacy_names.add(primary)
        if {"fat_loss", "muscle_gain"}.issubset(legacy_names):
            figure_goal = "recomposition"
        elif "muscle_gain" in legacy_names:
            figure_goal = "muscle_shape"
        else:
            figure_goal = "fat_loss_shape"
        details.setdefault("figure_goal", figure_goal)
        cleaned = [dict(g) for g in goals if str(g.get("goal")) not in LEGACY_FIGURE_GOALS]
        existing_figure = next((g for g in cleaned if str(g.get("goal")) == "figure"), None)
        legacy_priority = min([int(g.get("priority", 2)) for g in legacy] or [1 if has_legacy_primary else 2])
        if existing_figure:
            existing_figure["priority"] = min(int(existing_figure.get("priority", 2)), legacy_priority)
        else:
            cleaned.append({"goal": "figure", "priority": legacy_priority, "event_date": None, "target": None, "notes": None})
        new_primary = "figure" if has_legacy_primary else primary
        conn.execute(
            "UPDATE clients SET primary_goal=?,goals_json=?,goal_details_json=?,updated_at=? WHERE telegram_user_id=?",
            (new_primary, json.dumps(cleaned, ensure_ascii=False), json.dumps(details, ensure_ascii=False), now, row["telegram_user_id"]),
        )
    conn.execute("UPDATE programs SET goal='figure',updated_at=? WHERE goal IN ('fat_loss','muscle_gain')", (now,))
    conn.execute("UPDATE training_cycles SET sport='figure',updated_at=? WHERE sport IN ('fat_loss','muscle_gain')", (now,))
...
def build_plan(req: PlanRequest) -> PlanPreview:
    ...
    elif req.goal == Goal.figure:
        strength = 3 if req.days_per_week <= 4 else 4
        blocks = [
            TrainingBlock(name="Гипертрофия · низ тела", sessions=2, priority="primary", note="Ягодичные, ноги, задняя поверхность бедра; прогрессивный объём без отказа"),
            TrainingBlock(name="Гипертрофия · верх тела", sessions=1 if strength == 3 else 2, priority="primary", note="Спина, плечи, грудь и руки для пропорций фигуры"),
            TrainingBlock(name="Корпус", sessions=2, priority="support", note="Стабилизация, пресс и контроль таза"),
            TrainingBlock(name="Zone 2", sessions=2, priority="support", note="Низкоударная аэробная работа для общей активности и восстановления"),
...
def upsert_athlete_metric(conn, client_id: str, payload: AthleteMetricInput, *, source: str, changed_by: str, confirmed: bool):
    measured_at = payload.measured_at or app_today().isoformat()
    unit = (payload.unit or metric_default_unit(payload.metric_type)).strip()
    note = payload.note or None
    confirmed_int = 1 if confirmed else 0
    now = utc_now()

    # v0.6.10: do not create a second history row when the user presses Save
    # with exactly the same value/state. A real change (value/date/source/
    # confirmation/note) still creates a new history event.
    previous = conn.execute(
        "SELECT * FROM athlete_metrics WHERE client_id=? AND metric_type=?",
        (client_id, payload.metric_type),
    ).fetchone()
    same_as_current = False
    if previous:
        try:
            same_value = abs(float(previous["value"]) - float(payload.value)) < 1e-9
        except Exception:
            same_value = str(previous["value"]) == str(payload.value)
        same_as_current = (
            same_value
            and str(previous["unit"] or "") == unit
            and str(previous["measured_at"] or "") == measured_at
            and str(previous["source"] or "") == source
            and int(previous["confirmed"] or 0) == confirmed_int
            and (previous["note"] or None) == note
        )
    ...
    if not same_as_current:
        conn.execute(
            ...
            VALUES(?,?,?,?,?,?,?,?,?,?)""",
            (client_id, payload.metric_type, payload.value, unit, measured_at, source, confirmed_int, note, changed_by, now),
        )
    conn.execute(
...
def dedupe_athlete_metric_history(rows) -> list[dict]:
    """Hide exact legacy duplicate history rows while preserving real changes."""
    out = []
    seen = set()
    for row in rows:
        d = dict(row)
        key = (
            d.get("metric_type"), str(d.get("value")), d.get("unit") or "",
            d.get("measured_at") or "", d.get("source") or "",
            int(d.get("confirmed") or 0), d.get("note") or "",
        )
        if key in seen:
            continue
        seen.add(key)
...
def next_testing_start(conn, client_id: str) -> date:
    ...
    return (last + timedelta(days=1)) if last else app_today()
...
async def send_client_message(client_id: str, text: str) -> dict:
    """Отправляет Telegram-уведомление только целевому клиенту."""
    with db_conn() as conn:
        row = conn.execute("SELECT telegram_user_id,name FROM clients WHERE telegram_user_id=?", (str(client_id),)).fetchone()
    if not row:
        return {"sent": False, "reason": "client_not_found"}
    target_id = str(row["telegram_user_id"])
    # Автоматические клиентские уведомления не отправляем на тренерские аккаунты.
    # Это защищает от ситуации, когда тестовый профиль тренера выбран как клиент.
    if target_id in COACH_IDS:
        print(f"Client notification suppressed for coach account {target_id}")
        return {"sent": False, "reason": "target_is_coach"}
    try:
        await telegram_api("sendMessage", {"chat_id": int(target_id), "text": text})
        return {"sent": True, "chat_id": target_id}
    except Exception as exc:
        print(f"Client Telegram notification failed for {target_id}: {exc}")
...
GOAL_LABELS = {
    ...
    "fat_loss": "Фигура / композиция тела", "muscle_gain": "Фигура / композиция тела", "strength": "Сила", "figure": "Фигура / композиция тела",
}
...
def workout_text(kind: str, level: str, week: int, focus: list[str]) -> str:
    ...
    if kind == "figure_lower_a":
        return "Разминка 8–10 мин.\nПрисед с гирей у груди 3×10 @ ИВН 6–7.\nЯгодичный мост со штангой 3×10 @ ИВН 7.\nВыпады назад 2×10/10.\nОтведение бедра в кроссовере 2×15/15.\nКорпус: мёртвый жук 3×8/8.\nОтдых 60–120 сек. Оставлять 2–3 повтора в запасе."
    if kind == "figure_upper":
        return "Разминка плеч 8–10 мин.\nЖим гантелей лёжа 3×10 @ ИВН 6–7.\nВертикальная тяга в кроссовере 3×10–12.\nТяга гантели к поясу 3×10/10.\nЖим гантелей стоя 3×10.\nРазведения на среднюю/заднюю дельту 2×12–15.\nРазгибание бедра в кроссовере 2×12/12.\nZone 2 15–20 мин по самочувствию."
    if kind == "figure_lower_b":
        return "Разминка 8–10 мин.\nРумынская тяга 3×10 @ ИВН 6–7.\nБолгарские приседания 3×8/8.\nЗашагивания на тумбу 2×10/10.\nЯгодичный мост 2×12.\nОтведение бедра в кроссовере 2×15/15.\nБоковая планка 3×30 сек/сторона.\nФинишер: 8–10 мин умеренно на Байк Эрг / гребле, ИВН 6."
...
def weekly_kinds(row) -> list[str]:
    ...
    if primary == "crossfit":
        ...
        return variants[days]
    if primary == "figure" or "figure" in all_goals:
        seq = ["figure_lower_a", "figure_upper", "figure_lower_b", "zone2", "figure_lower_a", "figure_upper", "zone2"]
    elif primary == "muscle_gain":
...
TRAINING_SPORT_LABELS = {
    ...
    "fat_loss": "Фигура / композиция тела",
    "muscle_gain": "Фигура / композиция тела",
    "figure": "Фигура / композиция тела",
...
def training_cycle_dict(conn, row, include_workouts=True, coach_id: Optional[str] = None) -> dict:
    ...
    if include_workouts:
        ...
        for w in ws:
            ...
            if coach_id is not None:
                links = conn.execute(
                    """SELECT l.exercise_code,l.source_line,l.line_no,l.match_type,l.confidence,e.name_ru
                       FROM workout_exercise_links l
                       LEFT JOIN exercise_library e ON e.owner_telegram_id=l.owner_telegram_id AND e.exercise_code=l.exercise_code
                       WHERE l.owner_telegram_id=? AND l.source_type='cycle' AND l.source_id=?
                       ORDER BY l.line_no,l.exercise_code""",
                    (str(coach_id), int(w["id"])),
                ).fetchall()
                item["exercise_links"] = [dict(x) for x in links]
                item["exercise_link_count"] = len(links)
...
def assign_cycle_to_client(conn, cycle_row, workouts, client_row, start: date, replace_future: bool, activate_program: bool, owner_telegram_id: Optional[str] = None):
    if replace_future:
    ...
    for w in workouts:
        ...
        if load.get("rule"):
            full_text += "\n\nПравило нагрузки:\n" + load["rule"]
        wid = insert_workout(conn, client_row["telegram_user_id"], program_id, planned.isoformat(), w["title"], full_text)
        owner = str(owner_telegram_id or cycle_row["owner_telegram_id"] or "")
        if wid and owner:
            link_workout_text(conn, owner, "assignment", int(wid), normalize_workout_text_ru(full_text), preserve_manual=False)
        count += 1
    return program_id, count


# ---- v0.6.19: системные блоки тренировочных циклов M17 ----
# Циклы храним блоками по 4 недели. Это позволяет позднее связывать блоки
# автоматической прогрессией, не смешивая 12 недель в один монолитный шаблон.
M17_CF_BEGINNER_BLOCK1_KEY = "M17_CF_BEGINNER_BLOCK1_V1"
M17_CF_BEGINNER_BLOCK1_WORKOUTS = [
    {
        "week_no": 1, "day_index": 0, "position": 1,
        "title": "ПН · База приседа, тяга и корпус",
        "stimulus": "Спокойное освоение базовых паттернов. ИВН 5–6; в круговой части не выше 6. Качество движения важнее количества кругов.",
        "load_rule": "Оставлять не менее 3 повторов в запасе. Если техника нестабильна — уменьшить вес или использовать более простую регрессию из библиотеки.",
        "tags": ["адаптация", "присед", "тяга", "корпус", "МФР", "мобильность"],
        "text": """МФР — 6–8 минут
МФР икроножных на ролле — 45 сек/сторона
МФР квадрицепса на ролле — 45 сек/сторона
МФР ягодичных на ролле — 45 сек/сторона
МФР грудного отдела на ролле — 60 сек

Мобильность и активация — 2 круга
Мобилизация голеностопа у стены — 8/8
Переходы 90/90 — 6/6
Казачий присед — 6/6
Ягодичный мост с собственным весом — 12
Мёртвый жук — 6/6

Силовая и техника
Гоблет-присед — 3×8, ИВН 5, отдых 75–90 сек
Тяга гантели одной рукой — 3×10/10, ИВН 5–6
Отжимания от высокой опоры — 3×8–12, оставить минимум 3 повтора в запасе

Корпус — 3 круга спокойно
Птица-собака — 8/8
Боковая планка с колен — 20 сек/сторона
Перенос гири в одной руке «чемодан» — 20 м/сторона

Круговая работа — 8 минут, максимум качественных кругов
Гоблет-присед — 6
Тяга на кольцах — 8
Заходы на тумбу — 8
Гребля на эргометре — 6 калорий
Цель: ИВН 6. Зафиксировать круги + повторы.

Восстановление — 8–10 минут
Растяжка сгибателей бедра — 40 сек/сторона
Растяжка ягодичных — 40 сек/сторона
Растяжка икроножных — 40 сек/сторона
Растяжка широчайших в позе ребёнка — 60 сек
Диафрагмальное дыхание лёжа — 3 минуты, выдох длиннее вдоха"""
    },
    {
        "week_no": 1, "day_index": 2, "position": 1,
        "title": "СР · Наклон, плечевой пояс и аэробная база",
        "stimulus": "Техническая тренировка средней продолжительности. ИВН силовой части 5–6, аэробной — 4–5; разговорный темп.",
        "load_rule": "Румынскую тягу выполнять с 3-секундным опусканием. В аэробной части не ускоряться выше устойчивой зоны 2.",
        "tags": ["наклон", "верх тела", "координация", "аэробная база", "восстановление"],
        "text": """Разогрев
Байк Эрг — 5 минут легко

Динамическая мобильность — 2 круга
Кошка-корова — 8
Ротация грудного отдела — 6/6
Доброе утро без веса — 10
Выпад назад с подъёмом рук — 6/6
Лопаточные отжимания — 10
Тяга каната к лицу — 12

Силовая часть
Румынская тяга с гантелями — 3×8, ИВН 5–6, 3 сек вниз
Жим гантелей лёжа — 3×10, ИВН 5–6
Пулдаун прямыми руками — 3×12–15

Координация — 3 круга без спешки
Стойка на одной ноге — 20 сек/сторона
Наклон на одной ноге с касанием пола противоположной рукой — 5/5
Медвежья ходьба — 10 м
Прогулка фермера — 30 м

Аэробная работа — 24 минуты, ИВН 4–5
Байк Эрг — 8 минут
Гребля на эргометре — 8 минут
Лыжный эргометр или ходьба на дорожке — 8 минут
Переходы спокойные, разговорный темп.

Восстановление
МФР широчайших на ролле — 45 сек/сторона
МФР грудного отдела на ролле — 60 сек
МФР ягодичных на ролле — 45 сек/сторона
Диафрагмальное дыхание лёжа — 4 минуты"""
    },
    {
        "week_no": 1, "day_index": 4, "position": 1,
        "title": "ПТ · Одноногая сила, жим и смешанная работа",
        "stimulus": "Освоение одноногой работы и устойчивого корпуса. Комплекс выполнять ровно, без гонки; верхний предел ИВН 7.",
        "load_rule": "В силовых упражнениях оставлять 2–3 повтора в запасе. Если таз или колено теряют контроль — уменьшить амплитуду/вес.",
        "tags": ["одноногая сила", "жим", "тяга", "переносы", "профилактика"],
        "text": """Подготовка — 2 круга
Гребля на эргометре — 250 м легко
Выпады в ходьбе — 8/8
Присед с собственным весом — 10
Разведение резиновой ленты перед грудью — 10
Лопаточные подтягивания — 8
Планка — 20 сек

Силовая часть
Сплит-присед — 3×8/8, ИВН 5–6
Жим гантели одной рукой стоя — 3×8/8
Тяга гантелей лёжа на наклонной скамье — 3×10

Профилактический блок — 2–3 круга
Наружная ротация плеча с резиной — 12/12
Подъём на носки стоя — 15
Антиротационное удержание с резиной — 20 сек/сторона

Смешанный комплекс — 3 раунда
Гребля на эргометре — 200 м
Выпады назад с гантелями — 10 попеременно, лёгкий вес
Жим гантелей стоя — 8
Прогулка фермера — 20 м
Цель: ИВН 6–7, одинаковый темп всех раундов.
...
Спокойное дыхание — 2 минуты"""
    ...
    {
        "week_no": 2, "day_index": 0, "position": 1,
        "title": "ПН · Темповый присед и задняя цепь",
        "stimulus": "Небольшое увеличение общего объёма без усложнения навыков. ИВН 6; круговая работа 6–7.",
        "load_rule": "Сначала выдерживать темп 3-1-1. Вес увеличивать только если все повторы одинаковы и ИВН не выше 6.",
        "tags": ["темповая сила", "задняя цепь", "корпус", "МФР", "мобильность"],
        "text": """МФР — 5–7 минут
МФР подошвенной поверхности стопы массажным мячом — 45 сек/сторона
МФР икроножных на ролле — 45 сек/сторона
МФР квадрицепса на ролле — 45 сек/сторона
МФР ягодичных на ролле — 45 сек/сторона

Мобильность — 2 круга
Мобилизация голеностопа у стены — 10/10
Удержание глубокого приседа с опорой — 20 сек
Казачий присед — 6/6
Боковые шаги с мини-лентой — 10/10

Силовая часть
Темповый присед 3-1-1 — 4×8, ИВН 6
Ягодичный мост со штангой с пола — 3×12, лёгкий рабочий вес
Горизонтальная тяга блока нейтральным хватом — 3×10/10

Корпус
Мёртвый жук — 3×8/8
Боковая планка — 3×20–25 сек/сторона

Круговая работа — 9 минут
Присед на тумбу — 8
Тяга на кольцах — 8
Байк Эрг — 8 калорий
Перенос гири в одной руке «чемодан» — 20 м, менять руку каждый круг
Цель: ИВН 6–7, без потери техники.

Восстановление — 10 минут
Мобильность тазобедренных суставов
Мобильность голеностопа
Лёгкое МФР ягодичных и икроножных"""
    },
    {
        "week_no": 2, "day_index": 2, "position": 1,
        "title": "СР · Тяговый паттерн и обучение мощности",
        "stimulus": "Обучить безопасной тяге с пола и первому простому движению мощности. ИВН силовой части 6, интервальной — около 6.",
        "load_rule": "Взятие медицинского мяча выполнять только с весом, позволяющим сохранять последовательность разгибания таза и приёма. Скорость вторична.",
        "tags": ["тяга", "мощность", "медбол", "интервалы", "восстановление"],
        "text": """Разминка
Гребля на эргометре — 5 минут легко

Динамическая подготовка — 2 круга
Доброе утро без веса — 10
Выпад с ротацией грудного отдела — 6/6
Ягодичный мост на одной ноге — 8/8
Лопаточные отжимания — 10
Тяга каната к лицу — 15

Силовая часть
Становая тяга с гирей — 4×8, ИВН 6

Обучение мощности
Взятие медицинского мяча на грудь — 5×5, небольшой вес, полный контроль техники

Верх тела
Жим гантелей сидя — 3×10
Пулдаун прямыми руками — 3×15

Интервальная работа — 12 минут
4 раунда: 2 минуты умеренной работы + 1 минута очень легко
Чередование: Гребля на эргометре / Байк Эрг
Рабочая часть — ИВН 6.
...
Диафрагмальное дыхание лёжа — 3 минуты"""
    },
    {
        "week_no": 2, "day_index": 4, "position": 1,
        "title": "ПТ · Одноногая сила, хват и координация",
        "stimulus": "Усложнение одноногой силы относительно первой недели, но без роста общей интенсивности выше ИВН 7.",
        "load_rule": "Высоту задней опоры и вес болгарского приседа выбирать по контролю таза и колена. В переносах прекращать подход до распада хвата.",
        "tags": ["одноногая сила", "хват", "координация", "медбол", "растяжка"],
        "text": """Мобильность — 2 круга
Переходы 90/90 — 8/8
Выпад с ротацией грудного отдела — 5/5
Присед с вытяжением рук вверх — 8
Боковые шаги с мини-лентой — 10/10
Разведение резиновой ленты перед грудью — 12

Силовая часть
Болгарский присед с гантелью у груди — 3×8/8, ИВН 6
Отжимания от опоры — 3×8–12
Тяга гантели одной рукой — 3×10/10

Хват и переносы — 3 круга
Прогулка фермера — 30 м
Удержание гирь стоя — 30 сек
Отдых — 45 сек

Круговая работа — 10 минут
Лыжный эргометр — 8 калорий
Заходы на тумбу — 8
Взятие медицинского мяча на грудь — 6
Отжимания от опоры — 6
Цель: непрерывная ровная работа, ИВН 6–7.

Восстановление
МФР квадрицепса на ролле
МФР ягодичных на ролле
Статическая растяжка нижней части тела — 8–10 минут"""
    },
    {
        "week_no": 3, "day_index": 0, "position": 1,
        "title": "ПН · Контроль приседа и интервальная устойчивость",
        "stimulus": "Самая объёмная неделя блока. ИВН силовой части 6–7. В интервалах сохранить падение результата не более 10–15%.",
        "load_rule": "Если предыдущая неделя прошла технически чисто и ИВН не выше 6 — увеличить вес минимальным шагом. Иначе сохранить нагрузку.",
        "tags": ["присед", "задняя цепь", "интервалы", "контроль темпа", "восстановление"],
        "text": """Подготовка — 12 минут
МФР икроножных на ролле
МФР ягодичных на ролле
Мобилизация голеностопа у стены — 8/8
Казачий присед — 8/8
Ягодичный мост с собственным весом — 12
Присед с паузой без веса — 8
Мёртвый жук — 8/8

Силовая часть
Темповый присед 3-1-1 — 4×8, ИВН 6–7
Румынская тяга с гантелями — 4×8, ИВН 6–7
Тяга гантелей лёжа на наклонной скамье — 3×12

Интервалы — 5 раундов
Гребля на эргометре — 1 минута
Гоблет-присед — 8
Отдых — 1 минута
Записывать калории каждого интервала. Цель: падение производительности не более 10–15%.
...
Диафрагмальное дыхание лёжа"""
    ...
    {
        "week_no": 3, "day_index": 2, "position": 1,
        "title": "СР · Тяга со штангой, жим и баланс",
        "stimulus": "Переход к штанге только при готовности техники. Основная аэробная работа остаётся лёгкой; силовая часть ИВН 5–6.",
        "load_rule": "Если тяга с пола ещё нестабильна — вместо штанги оставить становую тягу с гирей. Номер недели сам по себе не является основанием для прогрессии.",
        "tags": ["становая тяга", "жим", "баланс", "аэробная база", "мобильность"],
        "text": """Разогрев
Байк Эрг — 5 минут легко

Динамическая разминка — 2 круга
Доброе утро без веса — 10
Птица-собака — 8/8
Выпад назад с подъёмом колена — 6/6
Лопаточные подтягивания — 8
Тяга каната к лицу — 12

Техника и сила
Если техника стабильна: Становая тяга со штангой классическая — 4×6, ИВН 5–6
Если техника нестабильна: Становая тяга с гирей — 4×8, ИВН 5–6
Жим двух гантелей стоя — 4×8, ИВН 6

Баланс и проприоцепция — 3 круга
Румынская тяга на одной ноге с гантелью — 6/6, лёгкий вес
Боковой заход на тумбу — 8/8
Перенос гири в одной руке «чемодан» — 20 м/20 м

Аэробная работа — 30 минут, зона 2, ИВН 4–5
Гребля на эргометре — 10 минут
Байк Эрг — 10 минут
Лыжный эргометр или ходьба на дорожке — 10 минут

Восстановление — 10–12 минут
...
Растяжка сгибателей бедра"""
    ...
    {
        "week_no": 3, "day_index": 4, "position": 1,
        "title": "ПТ · Обучение мощности и ровные раунды",
        "stimulus": "Закрепить разгибание таза и научить сохранять одинаковый темп в смешанной работе. Верхний предел ИВН 7.",
        "load_rule": "Мах гири разрешать только при уверенном наклоне и разгибании таза. Если техника распадается — уменьшить вес либо заменить на становую тягу с гирей.",
        "tags": ["мощность", "гиря", "одноногая сила", "ровный темп", "МФР"],
        "text": """МФР и мобильность — 8 минут
МФР ягодичных на ролле
МФР грудного отдела на ролле
Переходы 90/90
Мобилизация голеностопа у стены

Обучение мощности
Взятие медицинского мяча на грудь — 4×6
Мах гири двумя руками — 4×8, лёгкий вес, до уровня груди

Одноногая сила
Болгарский присед с гантелью у груди — 3×10/10

Плечевой пояс
Пулдаун прямыми руками — 3×15
Лопаточные отжимания — 3×10

Основной комплекс — 4 раунда
Гребля на эргометре — 250 м
Мах гири двумя руками — 8
Выпады назад с гантелями — 10 попеременно, лёгкий вес
Отжимания от опоры — 8
Прогулка фермера — 30 м
После каждого раунда отдых 60 сек.
Цель: одинаковое время всех четырёх раундов, ИВН 7.

Восстановление — 12 минут
МФР широчайших на ролле
МФР ягодичных на ролле
МФР икроножных на ролле
Мобильность грудного отдела
...
Диафрагмальное дыхание лёжа — 3 минуты"""
    ...
    {
        "week_no": 4, "day_index": 0, "position": 1,
        "title": "ПН · Разгрузка и качество движения",
        "stimulus": "Разгрузочная тренировка. Общий объём ниже примерно на 30–40%; цель — качество базовых движений и восстановление.",
        "load_rule": "Не повышать веса. Все силовые упражнения выполнять около ИВН 5, оставляя 3–4 повтора в запасе.",
        "tags": ["разгрузка", "техника", "профилактика", "МФР", "восстановление"],
        "text": """МФР — 8 минут
МФР подошвенной поверхности стопы массажным мячом
МФР икроножных на ролле
МФР квадрицепса на ролле
МФР ягодичных на ролле
МФР широчайших на ролле
...
Контроль техники
Гоблет-присед — 3×8, ИВН 5
Румынская тяга с гантелями — 3×8, ИВН 5
Тяга гантели одной рукой — 2×10/10
Отжимания от опоры — 2 подхода, оставить 3–4 повтора в запасе

Коррекционный блок — 2 круга
Птица-собака — 8/8
Боковая планка — 20 сек/сторона
Тяга каната к лицу — 12
Подъём на носки стоя — 15

Лёгкая кондиционная работа
Гребля на эргометре или Байк Эрг — 12 минут непрерывно, ИВН 4–5

Восстановление — 15 минут
Мобильность тазобедренных суставов
Мобильность голеностопа
МФР ягодичных и икроножных
Спокойное дыхание"""
    },
    {
        "week_no": 4, "day_index": 2, "position": 1,
        "title": "СР · Аэробный контроль и восстановление",
        "stimulus": "Контроль аэробной устойчивости без предельной работы. После теста — полноценное активное восстановление и мобильность.",
        "load_rule": "Шестиминутный тест выполнять ровно на ИВН 7–8, без стартового спринта. Использовать тот же эргометр при последующих повторных тестах.",
        "tags": ["контроль", "аэробная база", "активное восстановление", "мобильность", "МФР"],
        "text": """Постепенный разогрев — 10 минут
5 минут легко
3 минуты умеренно
2 минуты снова легко

Аэробный контроль — 6 минут
Гребля на эргометре или Байк Эрг — выбрать один вариант и сохранять его для следующих тестов
Работать максимально ровно, ИВН 7–8.
Зафиксировать дистанцию или калории, средний темп, средний пульс при наличии и ИВН после теста.

Активное восстановление
Байк Эрг, гребля или ходьба — 15–20 минут, зона 1–2

Мобильность — 15 минут
Мобилизация голеностопа у стены
Переходы 90/90
Растяжка задней поверхности бедра
Мобильность грудного отдела
...
Диафрагмальное дыхание лёжа — 5 минут, спокойный длинный выдох"""
    },
    {
        "week_no": 4, "day_index": 4, "position": 1,
        "title": "ПТ · Контрольный комплекс №1",
        "stimulus": "Повторить стартовый комплекс в тех же условиях. Улучшение оценивается по результату, ИВН, технике и восстановлению.",
        "load_rule": "Использовать тот же вес в гоблет-приседе и ту же высоту тумбы, что в первую неделю. Не менять условия ради лучшего результата.",
        "tags": ["контроль", "присед", "тяга", "эргометр", "восстановление"],
        "text": """Подготовка — 12–15 минут без утомления
Мобилизация голеностопа у стены
Переходы 90/90
Ротация грудного отдела
Гребля на эргометре — легко

Активация — 2 круга
Гоблет-присед — 6
Тяга на кольцах — 6
Заходы на тумбу — 6
Гребля на эргометре — 4 калории
После активации отдых 3 минуты.

Контрольный комплекс №1 — 8 минут, максимум качественных кругов
Гоблет-присед — 6
Тяга на кольцах — 8
Заходы на тумбу — 8
Гребля на эргометре — 6 калорий
Использовать тот же вес и ту же высоту тумбы, что в первую неделю.

Зафиксировать
Круги + повторы
ИВН
Качество техники
Средний пульс при наличии
Восстановление через 2 минуты

После теста — 15 минут
Лёгкая ходьба или Байк Эрг
МФР ягодичных и икроножных
Статическая растяжка нижней части тела
Диафрагмальное дыхание лёжа"""
    },
]


def seed_m17_cf_beginner_block1(conn, owner_telegram_id: str) -> int:
    """Однократно добавляет тренеру редактируемый блок КФ Новичок 1–4 недели."""
    owner = str(owner_telegram_id or "").strip()
    if not owner:
        raise ValueError("owner_telegram_id required")
    title = "КФ · Новичок · Блок 1 · Адаптация (1–4 недели)"
    notes = (
        "Блок 1 из 3 в 12-недельной системе КФ Новичок. График: ПН / СР / ПТ. "
        "В каждую неделю встроены МФР, мобильность, силовая/техническая работа, кондиционная часть и восстановление. "
        f"Системный ключ: {M17_CF_BEGINNER_BLOCK1_KEY}."
    )
    row = conn.execute(
        "SELECT * FROM training_cycles WHERE owner_telegram_id=? AND notes LIKE ? ORDER BY id LIMIT 1",
        (owner, f"%{M17_CF_BEGINNER_BLOCK1_KEY}%"),
    ).fetchone()
    if row:
        # После первого добавления шаблон становится собственным циклом тренера.
        # Его ручные изменения не перезаписываем при следующих деплоях/рестартах.
        return int(row["id"])
    payload = TrainingCycleCreate(
        title=title, sport="crossfit", level="beginner", audience="all",
        duration_weeks=4, sessions_per_week=3,
        phase="Блок 1 · адаптация и базовые паттерны",
        notes=notes, source="manual",
    )
    cycle_id = create_training_cycle(conn, payload, owner)
    for raw in M17_CF_BEGINNER_BLOCK1_WORKOUTS:
        add_cycle_workout(conn, cycle_id, TrainingCycleWorkoutCreate(**raw))
    return cycle_id
...
async def startup():
    ...
    try:
        seeded = []
        with db_conn() as conn:
            for owner in sorted(COACH_IDS):
                seeded.append((owner, seed_m17_cf_beginner_block1(conn, str(owner))))
            conn.commit()
        if seeded:
            print(f"M17 coach cycles ready: {seeded}")
    except Exception as exc:
        print(f"M17 cycle seed failed: {exc}")
    try:
        repair_incomplete_back_block()
    except Exception as exc:
        print(f"BACK block repair failed: {exc}")
    # v0.6.16: после деплоя повторно связываем уже существующие циклы и
    # назначения с полной библиотекой упражнений. Ручные связи сохраняются.
    try:
        for owner in sorted(COACH_IDS):
            with db_conn() as conn:
                report = link_existing_exercises_for_coach(conn, str(owner), max_unmatched=0)
                conn.commit()
            print(
                f"Exercise/program integration owner={owner}: "
                f"cycles={report.get('cycle_workouts', 0)}, "
                f"assignments={report.get('assigned_workouts', 0)}, "
                f"linked={report.get('linked_exercises', 0)}, "
                f"unmatched={report.get('unmatched_count', 0)}"
            )
    except Exception as exc:
        print(f"Exercise/program integration failed: {exc}")
...
def health():
    return {
        ...
        "timezone": APP_TIMEZONE_NAME, "app_date": app_today().isoformat(),
        "features": ["multi_goal", "training_days", "triathlon_distances", "running_distances", "trail_running", "sequential_workouts", "vacation_mode", "subscriptions", "mandatory_feedback", "live_edit", "athlete_metrics", "training_library", "telegram_json_import", "bulk_text_import", "cycle_assignment", "1rm", "ftp", "thresholds", "two_week_testing", "figure_single_branch", "client_targeted_notifications", "exercise_library", "exercise_library_import", "exercise_visuals", "client_exercise_technique", "legacy_exercise_linking", "singapore_workout_day_unlock", "mandatory_feedback_flow", "coach_feedback_notifications", "exercise_back_block_self_repair", "unlimited_exercise_visuals", "exercise_program_auto_integration", "client_workout_exercise_list", "exercise_visual_prompt_generator", "batch_visual_prompt_export", "visual_correction_prompt", "visual_workflow_queue", "coach_client_visual_zoom", "program_blocks", "system_cf_beginner_block1"],
    }
...
def client_home(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        if current and access["allowed"]:
            ...
            if cdate and cdate <= app_today():
                current_view = client_workout_view_with_exercises(conn, current, include_content=(current["status"] == "OPENED"))
...
def client_workouts(x_telegram_init_data: Optional[str] = Header(default=None)):
    """Только текущая доступная тренировка клиента + связанные карточки техники."""
    user = current_user(x_telegram_init_data, required=True)
    uid = str(user["id"])
    current_view = None
    next_date = None
...
def client_archive(x_telegram_init_data: Optional[str] = Header(default=None)):
    """Личный архив закрытых тренировок с доступом к технике упражнений."""
    user = current_user(x_telegram_init_data, required=True)
    uid = str(user["id"])
    with db_conn() as conn:
        access = access_state(conn, uid, coach_override=is_coach_id(uid))
        ...
        items = []
        for row in rows:
            item = client_workout_view_with_exercises(conn, row, include_content=True)
...
def open_workout(workout_id: int, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        workout_view = client_workout_view_with_exercises(conn, row, include_content=True)
        conn.commit()
    return {"ok": True, "workout": workout_view}


@app.get("/client/workouts/{workout_id}/exercises/{exercise_code}")
def client_exercise_technique(workout_id: int, exercise_code: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    user = current_user(x_telegram_init_data, required=True)
    uid = str(user["id"])
    with db_conn() as conn:
        workout = conn.execute("SELECT * FROM workout_assignments WHERE id=? AND client_id=?", (workout_id, uid)).fetchone()
        if not workout:
            raise HTTPException(status_code=404, detail="Тренировка не найдена")
        # Техника доступна только для упражнения, реально связанного с тренировкой клиента.
        ensure_assignment_exercise_links(conn, workout)
        link = conn.execute(
            "SELECT * FROM workout_exercise_links WHERE source_type='assignment' AND source_id=? AND exercise_code=? ORDER BY id LIMIT 1",
            (workout_id, exercise_code),
        ).fetchone()
        if not link:
            raise HTTPException(status_code=404, detail="Упражнение не связано с этой тренировкой")
        ex = conn.execute(
            "SELECT * FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?",
            (link["owner_telegram_id"], exercise_code),
        ).fetchone()
        if not ex:
            raise HTTPException(status_code=404, detail="Карточка упражнения не найдена")
        visuals = get_exercise_visuals(conn, str(link["owner_telegram_id"]), exercise_code)
        conn.commit()
    return {
        "exercise_code": ex["exercise_code"], "name_ru": ex["name_ru"], "family": ex["family"],
        "movement_pattern": ex["movement_pattern"], "primary_muscles": ex["primary_muscles"],
        "secondary_muscles": ex["secondary_muscles"], "equipment": ex["equipment"], "difficulty": ex["difficulty"],
        "load_method": ex["load_method"], "rpe_range": ex["rpe_range"], "rep_range": ex["rep_range"],
        "tempo": ex["tempo"], "rest_range": ex["rest_range"], "technique": ex["technique"],
        "common_errors": ex["common_errors"], "regression": ex["regression"], "progression": ex["progression"],
        "client_tip": ex["client_tip"], "visuals": visuals, "visual_count": len(visuals),
        "workout_assignment": extract_workout_prescription(link["source_line"]),
        "workout_line_no": int(link["line_no"]),
    }
...
async def submit_feedback(workout_id: int, payload: WorkoutFeedback, x_telegram_init_data: Optional[str] = Header(default=None)):
    user = current_user(x_telegram_init_data, required=True)
    uid = str(user["id"])
    ...
    with db_conn() as conn:
        ...
        # Normal flow: feedback closes an OPENED workout. Recovery flow: if an older UI
        # already moved it to archive, the client may still submit/correct feedback there.
        if previous_status not in {"OPENED", "COMPLETED", "PARTIAL", "NOT_DONE"}:
            raise HTTPException(status_code=409, detail="Сначала откройте текущую тренировку")
        client_row = conn.execute("SELECT name FROM clients WHERE telegram_user_id=?", (uid,)).fetchone()
        if client_row and client_row["name"]:
            client_name = client_row["name"]
        workout_title = row["title"] or "Тренировка"
        workout_date = row["workout_date"] or ""
        # One final feedback per workout; replace if client corrects it from archive.
        ...
        # The workout moves to archive only in the same transaction that saves feedback.
        conn.execute("UPDATE workout_assignments SET status=?,updated_at=? WHERE id=?", (payload.completion_status, utc_now(), workout_id))
        conn.commit()

    status_label = {"COMPLETED": "выполнена", "PARTIAL": "выполнена частично", "NOT_DONE": "не выполнена"}.get(payload.completion_status, payload.completion_status)
    lines = [
        "M17 · Обратная связь по тренировке",
        f"Клиент: {client_name}",
        f"Тренировка: {workout_title}",
        f"Дата: {workout_date}",
        f"Статус: {status_label}",
        f"ИВН / RPE: {payload.rpe if payload.rpe is not None else '—'}/10",
        f"Самочувствие: {payload.wellbeing}/10",
        f"Боль / дискомфорт: {'да' if payload.pain else 'нет'}",
    ]
    if payload.pain_location:
        lines.append(f"Где болит: {payload.pain_location}")
    if payload.reason:
        lines.append(f"Причина: {payload.reason}")
    if payload.comment:
        lines.append(f"Комментарий: {payload.comment}")
    alert_flag = bool(payload.pain or payload.wellbeing <= 3 or (payload.rpe or 0) >= 10)
    if alert_flag:
        lines.insert(1, "⚠️ Требует внимания")
    text = "\n".join(lines)

    notified = 0
    for coach_id in COACH_IDS:
        try:
            await telegram_api("sendMessage", {"chat_id": int(coach_id), "text": text})
            notified += 1
        except Exception as exc:
            print(f"Coach feedback notification failed for {coach_id}: {exc}")
        try:
            await notify_client(str(coach_id), {
                "type": "workout_feedback", "client_id": uid, "workout_id": workout_id,
                "status": payload.completion_status, "alert": alert_flag,
            })
        except Exception:
            pass

    return {
        "ok": True, "alert": alert_flag, "coach_notifications_sent": notified,
        "archived": True, "previous_status": previous_status,
    }
...
def get_client_athlete_metrics(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        assessment = athlete_assessment_state(conn, row)
    return {"metrics": snapshot, "history": dedupe_athlete_metric_history(history), "assessment": assessment, "meta": ATHLETE_METRIC_META}
    ...
    with db_conn() as conn:
        ...
        for r in rows:
            ...
            result.append({
                ...
                "sex": r["sex"], "client_type": r["client_type"],
...
def coach_client_detail(client_id: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    return {
        ...
        "athlete_metric_history": dedupe_athlete_metric_history(athlete_metric_history),
        "assessment": assessment, "feedback": [dict(x) for x in feedback], "workouts": ws,
    }


@app.delete("/coach/clients/{client_id}")
def coach_delete_client(client_id: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    with db_conn() as conn:
        row = conn.execute("SELECT telegram_user_id,name FROM clients WHERE telegram_user_id=?", (client_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Client not found")

        # Удаляем зависимые данные вручную: старые базы M17 создавались без ON DELETE CASCADE,
        # поэтому одинаково безопасно работает и SQLite, и PostgreSQL.
        # Сначала удаляем привязки упражнений к назначениям клиента (в таблице нет FK для обратной совместимости).
        assignment_ids = conn.execute("SELECT id FROM workout_assignments WHERE client_id=?", (client_id,)).fetchall()
        for a in assignment_ids:
            conn.execute("DELETE FROM workout_exercise_links WHERE source_type='assignment' AND source_id=?", (int(a["id"]),))
        ...
        cur = conn.execute("DELETE FROM clients WHERE telegram_user_id=?", (client_id,))
        if cur.rowcount == 0:
            raise HTTPException(status_code=404, detail="Client not found")
        conn.commit()

    # Убираем зарегистрированные live-соединения из локального реестра;
    # при следующем запросе удалённый профиль уже не авторизуется как клиент.
    WS_CONNECTIONS.pop(str(client_id), None)
    return {"ok": True, "client_id": client_id, "name": row["name"], "deleted": deleted}
...
async def coach_create_program(client_id: str, payload: ProgramCreate, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        if payload.generate_workouts:
            start = parse_iso_date(payload.start_date) or app_today()
            generate_intro_workouts(conn, row, program_id, start, weeks=4)
        conn.commit()
    await notify_client(client_id, {"type": "program_updated"})
    notification = await send_client_message(client_id, "Тренер назначил/обновил вашу программу M17. Откройте приложение, чтобы увидеть тренировки.")
    return {"ok": True, "program_id": program_id, "status": status, "plan": preview.model_dump(mode="json"), "notification": notification}
...
async def coach_add_workout(client_id: str, payload: WorkoutCreate, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        if wid:
            link_workout_text(conn, str(coach["id"]), "assignment", int(wid), normalize_workout_text_ru(payload.text), preserve_manual=False)
        conn.commit()
    await notify_client(client_id, {"type": "workout_updated", "workout_id": wid})
    notification = await send_client_message(client_id, f"Тренер добавил тренировку на {payload.workout_date}: {payload.title}.")
    return {"ok": True, "workout_id": wid, "notification": notification}
...
async def coach_edit_workout(workout_id: int, payload: WorkoutEdit, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        conn.execute("DELETE FROM workout_exercise_links WHERE source_type='assignment' AND source_id=?", (workout_id,))
        link_workout_text(conn, str(coach["id"]), "assignment", int(workout_id), normalize_workout_text_ru(new_text), preserve_manual=False)
        conn.commit()
        client_id = str(row["client_id"])
    await notify_client(client_id, {"type": "workout_updated", "workout_id": workout_id, "version": new_version})
    notification = await send_client_message(client_id, f"Тренер изменил тренировку «{new_title}» на {new_date}. Откройте M17, чтобы увидеть обновление.")
    return {"ok": True, "version": new_version, "notification": notification}




# ---- v0.6.8: exercise-library linking for existing cycles and client workouts ----
EXERCISE_MATCH_REPLACEMENTS = {
    "болгарские приседания": "болгарский присед",
    "болгарское приседание": "болгарский присед",
    "приседания": "присед",
    "приседание": "присед",
    "выпады": "выпад",
    "выпада": "выпад",
    "подтягивания": "подтягивание",
    "отжимания": "отжимание",
    "сгибания": "сгибание",
    "разгибания": "разгибание",
    "разведения": "разведение",
    "подъемы": "подъем",
    "подъёмы": "подъем",
    "зашагивания": "зашагивание",
    "маховые": "мах",
}
EXERCISE_EQUIPMENT_WORDS = {
    "гантель", "гантели", "гантелями", "штанга", "штангой", "гиря", "гирей", "гири", "гирями",
    "резина", "резиной", "резинкой", "блок", "блоке", "кроссовер", "кроссовере", "тренажер", "тренажере",
    "скамья", "скамье", "кольца", "кольцах", "канат", "каната", "рукоять", "рукоятью",
}
EXERCISE_STOP_WORDS = {
    "с", "со", "в", "на", "к", "ко", "из", "за", "для", "и", "или", "под", "над", "у", "по", "от",
    "одной", "одним", "двумя", "двух", "прямым", "обратным", "нейтральным", "узким", "широким",
}


def _canon_exercise_text(value: str) -> str:
    text = str(value or "").lower().replace("ё", "е")
    for old, new in EXERCISE_MATCH_REPLACEMENTS.items():
        text = text.replace(old, new)
    text = re.sub(r"\([^)]*\)", " ", text)
    # Убираем параметры подходов/нагрузки из строки, оставляя название движения.
    text = re.split(r"\s(?:\d+\s*[xх×]\s*\d+|\d+\s*подх|@\s*ивн|@\s*rpe|ивн\s*\d|rpe\s*\d)", text, maxsplit=1)[0]
    text = re.sub(r"\b\d+(?:[.,]\d+)?(?:\s*[-–]\s*\d+(?:[.,]\d+)?)?\b", " ", text)
    text = re.sub(r"[^a-zа-я0-9]+", " ", text)
    return " ".join(text.split()).strip()


def _exercise_tokens(value: str, drop_equipment: bool = False) -> set[str]:
    tokens = set(_canon_exercise_text(value).split())
    tokens -= EXERCISE_STOP_WORDS
    if drop_equipment:
        tokens -= EXERCISE_EQUIPMENT_WORDS
    return {x for x in tokens if len(x) >= 3}


def _exercise_aliases(name: str) -> list[str]:
    base = _canon_exercise_text(name)
    aliases = {base}
    # Упрощённый вариант без оборудования помогает связать старые строки типа
    # «Болгарские приседания 3×8/8» с полной карточкой библиотеки.
    tokens = [x for x in base.split() if x not in EXERCISE_EQUIPMENT_WORDS and x not in EXERCISE_STOP_WORDS]
    if len(tokens) >= 2:
        aliases.add(" ".join(tokens))
    for marker in [" с ", " со ", " в ", " на ", " у "]:
        if marker in base:
            left = base.split(marker, 1)[0].strip()
            significant = _exercise_tokens(left, drop_equipment=True)
            if len(left) >= 5 and len(significant) >= 2:
                aliases.add(left)
    return sorted((x for x in aliases if len(x) >= 4), key=len, reverse=True)


def _exercise_catalog(conn, owner: str) -> list[dict]:
    rows = conn.execute(
        "SELECT exercise_code,name_ru FROM exercise_library WHERE owner_telegram_id=? AND active=1 ORDER BY exercise_code",
        (owner,),
    ).fetchall()
    out = []
    for row in rows:
        out.append({
            ...
            "name_ru": row["name_ru"],
            "norm": _canon_exercise_text(row["name_ru"]),
            "aliases": _exercise_aliases(row["name_ru"]),
            "tokens": _exercise_tokens(row["name_ru"], drop_equipment=True),
        })
    return out


def _score_exercise_line(line: str, item: dict) -> float:
    norm = _canon_exercise_text(line)
    if not norm:
        return 0.0
    full = item["norm"]
    if full and (norm == full or norm.startswith(full + " ") or (len(full) >= 8 and full in norm)):
        return 0.999
    for alias in item["aliases"]:
        if alias == full:
            continue
        significant = _exercise_tokens(alias, drop_equipment=True)
        if len(significant) >= 2 and alias and (norm == alias or norm.startswith(alias + " ") or (len(alias) >= 10 and alias in norm)):
            return 0.90
    a_tokens = _exercise_tokens(norm, drop_equipment=True)
    b_tokens = item["tokens"]
    if not a_tokens or not b_tokens:
        return 0.0
    overlap = len(a_tokens & b_tokens) / max(1, len(a_tokens | b_tokens))
    containment = len(a_tokens & b_tokens) / max(1, min(len(a_tokens), len(b_tokens)))
    ratio = SequenceMatcher(None, norm, item["norm"]).ratio()
    return max(ratio * 0.62 + overlap * 0.23 + containment * 0.15, containment * 0.82 + ratio * 0.18)


def _workout_match_lines(text: str) -> list[tuple[int, str]]:
    ...
    for idx, raw in enumerate(lines, start=1):
        ...
        if low.startswith("целевой стимул") or low.startswith("правило нагрузки"):
            stop_section = True
            continue
        if stop_section:
            continue
        if low in {"разминка", "заминка", "основная часть", "кондиционная работа", "мобилити", "корпус"}:
            continue
        out.append((idx, line))
    return out


def link_workout_text(conn, owner: str, source_type: str, source_id: int, text: str, preserve_manual: bool = True) -> dict:
    """Автоматически связывает строки тренировки с карточками библиотеки.

    Ручные сопоставления сохраняются при повторной миграции. При редактировании
    конкретной тренировки вызывающий код может предварительно удалить все ссылки.
    """
    owner = str(owner or "").strip()
    if not owner:
        return {"linked_lines": 0, "linked_exercises": 0, "unmatched": [], "considered": 0}
    catalog = _exercise_catalog(conn, owner)
    if not catalog:
        return {"linked_lines": 0, "linked_exercises": 0, "unmatched": [], "considered": 0}
    if preserve_manual:
        conn.execute(
            "DELETE FROM workout_exercise_links WHERE owner_telegram_id=? AND source_type=? AND source_id=? AND match_type='AUTO'",
            (owner, source_type, source_id),
        )
    else:
        conn.execute(
            "DELETE FROM workout_exercise_links WHERE owner_telegram_id=? AND source_type=? AND source_id=?",
            (owner, source_type, source_id),
        )
    manual_lines = set()
    if preserve_manual:
        rows = conn.execute(
            "SELECT line_no FROM workout_exercise_links WHERE owner_telegram_id=? AND source_type=? AND source_id=? AND match_type='MANUAL'",
            (owner, source_type, source_id),
        ).fetchall()
        manual_lines = {int(r["line_no"]) for r in rows}
    now = utc_now()
    linked_lines = 0
    linked_exercises = 0
    unmatched = []
    considered = 0
    for line_no, line in _workout_match_lines(text):
        if line_no in manual_lines:
            linked_lines += 1
            continue
        ranked = sorted(((round(_score_exercise_line(line, item), 4), item) for item in catalog), key=lambda x: x[0], reverse=True)
        if not ranked:
            continue
        best_score, best_item = ranked[0]
        # Строки, которые совсем не похожи на силовое упражнение библиотеки, не считаем ошибкой миграции.
        if best_score < 0.44:
            continue
        considered += 1
        # Для точных совпадений допускаем несколько упражнений на одной строке; для fuzzy — только лучшее.
        matches = []
        if best_score >= 0.995:
            matches = [(score, item) for score, item in ranked if score >= 0.995]
        elif best_score >= 0.72:
            # Если второе совпадение почти равно первому, оставляем строку для ручной проверки, а не угадываем.
            second = ranked[1][0] if len(ranked) > 1 else 0
            if best_score - second >= 0.06:
                matches = [(best_score, best_item)]
        if matches:
            seen = set()
            for score, item in matches:
                ...
                try:
                    conn.execute(
                        "INSERT INTO workout_exercise_links(owner_telegram_id,source_type,source_id,line_no,source_line,exercise_code,match_type,confidence,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                        (owner, source_type, int(source_id), int(line_no), line, code, "AUTO", float(score), now, now),
                    )
                except Exception:
                    # Повторный запуск миграции должен быть безопасным даже на старых БД.
                    conn.execute(
                        "UPDATE workout_exercise_links SET source_line=?,confidence=?,updated_at=? WHERE owner_telegram_id=? AND source_type=? AND source_id=? AND line_no=? AND exercise_code=?",
                        (line, float(score), now, owner, source_type, int(source_id), int(line_no), code),
                    )
                linked_exercises += 1
            linked_lines += 1
        else:
            unmatched.append({
                ...
                "line_no": int(line_no),
                "source_line": line,
                "suggested_code": best_item["exercise_code"],
                "suggested_name": best_item["name_ru"],
                "confidence": round(float(best_score), 2),
            })
    return {"linked_lines": linked_lines, "linked_exercises": linked_exercises, "unmatched": unmatched, "considered": considered}


def assignment_exercise_owner(conn, row) -> Optional[str]:
    """Определяет библиотеку тренера для старого назначения без явной ссылки."""
    try:
        program_id = row["program_id"]
    except Exception:
        program_id = None
    if program_id:
        program = conn.execute("SELECT plan_json FROM programs WHERE id=?", (program_id,)).fetchone()
        if program:
            plan = safe_json(program["plan_json"], {})
            cycle_id = plan.get("cycle_id") if isinstance(plan, dict) else None
            if cycle_id:
                cycle = conn.execute("SELECT owner_telegram_id FROM training_cycles WHERE id=?", (cycle_id,)).fetchone()
                if cycle and cycle["owner_telegram_id"]:
                    return str(cycle["owner_telegram_id"])
    owners = conn.execute("SELECT DISTINCT owner_telegram_id FROM exercise_library WHERE active=1").fetchall()
    owner_values = [str(x["owner_telegram_id"]) for x in owners if x["owner_telegram_id"]]
    if len(owner_values) == 1:
        return owner_values[0]
    if len(COACH_IDS) == 1:
        return next(iter(COACH_IDS))
    return None


def ensure_assignment_exercise_links(conn, row) -> None:
    existing = conn.execute(
        "SELECT 1 AS ok FROM workout_exercise_links WHERE source_type='assignment' AND source_id=? LIMIT 1",
        (int(row["id"]),),
    ).fetchone()
    if existing:
        return
    owner = assignment_exercise_owner(conn, row)
    if not owner:
        return
    content = safe_json(row["content_json"], {})
    text = normalize_workout_text_ru(content.get("text", "") if isinstance(content, dict) else "")
    link_workout_text(conn, owner, "assignment", int(row["id"]), text, preserve_manual=True)


def extract_workout_prescription(source_line: str) -> dict:
    """Извлекает только очевидные параметры из конкретной строки тренировки.

    Полная строка всегда возвращается как ``assignment`` и остаётся источником
    истины. Остальные поля — удобные подсказки для клиентской карточки и могут
    быть пустыми, если формат тренировки свободный.
    """
    line = " ".join(str(source_line or "").split())
    out = {"assignment": line}
    if not line:
        return out

    patterns = {
        "sets_reps": r"\b\d+\s*[xх×]\s*\d+(?:\s*[-–]\s*\d+)?(?:\s*/\s*\d+(?:\s*[-–]\s*\d+)?)?\b|\b\d+\s*подх(?:од(?:а|ов)?)?\.?\s*(?:по\s*)?\d+(?:\s*[-–]\s*\d+)?",
        "intensity": r"(?:@\s*)?(?:ИВН|RPE)\s*[:=]?\s*\d+(?:[.,]\d+)?(?:\s*[-–]\s*\d+(?:[.,]\d+)?)?|\b\d+(?:[.,]\d+)?\s*%\s*(?:1ПМ|FTP)?|\b\d+(?:[.,]\d+)?\s*(?:кг|kg|Вт|W)\b|\b(?:Z|Зона)\s*[1-5]\b",
        "rest": r"(?:отдых|rest)\s*[:=\-]?\s*\d+(?:[.,]\d+)?\s*(?:сек|с|min|мин)",
        "tempo": r"(?:темп|tempo)\s*[:=\-]?\s*[0-9XxХх]+(?:\s*[-–/]\s*[0-9XxХх]+){1,3}",
        "duration_distance": r"\b\d+(?:[.,]\d+)?\s*(?:км|km|м|m|мин|min|сек|с)\b",
    }
    for key, pattern in patterns.items():
        m = re.search(pattern, line, flags=re.IGNORECASE)
        if m:
            out[key] = m.group(0).strip()
    # Не дублируем время отдыха как отдельную длительность упражнения.
    if out.get("duration_distance") and out.get("rest") and out["duration_distance"].lower() in out["rest"].lower():
        out.pop("duration_distance", None)
    return out


def workout_exercise_previews(conn, row) -> list[dict]:
    ensure_assignment_exercise_links(conn, row)
    rows = conn.execute(
        """SELECT l.line_no,l.source_line,l.exercise_code,l.owner_telegram_id,e.name_ru,e.family,e.equipment,
                  (SELECT COUNT(*) FROM exercise_visuals v WHERE v.owner_telegram_id=l.owner_telegram_id AND v.exercise_code=l.exercise_code) AS visual_count
           FROM workout_exercise_links l
           JOIN exercise_library e ON e.owner_telegram_id=l.owner_telegram_id AND e.exercise_code=l.exercise_code
           WHERE l.source_type='assignment' AND l.source_id=?
           ORDER BY l.line_no,l.id""",
        (int(row["id"]),),
    ).fetchall()
    out=[]; seen=set()
    for r in rows:
        key=(r["owner_telegram_id"],r["exercise_code"])
        if key in seen:
            continue
        seen.add(key)
        out.append({
            "line_no": int(r["line_no"]), "source_line": r["source_line"], "exercise_code": r["exercise_code"],
            "name_ru": r["name_ru"], "family": r["family"], "equipment": r["equipment"],
            "visual_count": int(r["visual_count"] or 0),
            "workout_parameters": extract_workout_prescription(r["source_line"]),
        })
    return out


def client_workout_view_with_exercises(conn, row, include_content: bool = False) -> Optional[dict]:
    item = client_workout_view(row, include_content=include_content)
    if item and include_content:
        item["exercise_links"] = workout_exercise_previews(conn, row)
    return item


def link_existing_exercises_for_coach(conn, owner: str, max_unmatched: int = 80) -> dict:
    totals = {"cycle_workouts": 0, "assigned_workouts": 0, "considered": 0, "linked_lines": 0, "linked_exercises": 0, "unmatched_count": 0}
    unmatched = []
    cycle_rows = conn.execute(
        """SELECT w.id,w.title,w.content_json FROM training_cycle_workouts w
           JOIN training_cycles c ON c.id=w.cycle_id
           WHERE c.owner_telegram_id=? OR c.owner_telegram_id IS NULL OR c.owner_telegram_id=''""",
        (owner,),
    ).fetchall()
    for row in cycle_rows:
        text = safe_json(row["content_json"], {}).get("text", "")
        r = link_workout_text(conn, owner, "cycle", int(row["id"]), text, preserve_manual=True)
        totals["cycle_workouts"] += 1
        totals["considered"] += r["considered"]
        totals["linked_lines"] += r["linked_lines"]
        totals["linked_exercises"] += r["linked_exercises"]
        totals["unmatched_count"] += len(r["unmatched"])
        for x in r["unmatched"]:
            if len(unmatched) < max_unmatched:
                x["source_title"] = row["title"]
                unmatched.append(x)
    assignment_rows = conn.execute("SELECT * FROM workout_assignments ORDER BY id").fetchall()
    for row in assignment_rows:
        # В текущей однокомандной архитектуре M17 упражнения клиента принадлежат библиотеке этого тренера.
        text = safe_json(row["content_json"], {}).get("text", "")
        r = link_workout_text(conn, owner, "assignment", int(row["id"]), normalize_workout_text_ru(text), preserve_manual=True)
        totals["assigned_workouts"] += 1
        totals["considered"] += r["considered"]
        totals["linked_lines"] += r["linked_lines"]
        totals["linked_exercises"] += r["linked_exercises"]
        totals["unmatched_count"] += len(r["unmatched"])
        for x in r["unmatched"]:
            if len(unmatched) < max_unmatched:
                x["source_title"] = row["title"]
                unmatched.append(x)
    return {**totals, "unmatched": unmatched, "unmatched_truncated": totals["unmatched_count"] > len(unmatched)}


EXERCISE_LIBRARY_FIELDS = [
    "name_ru", "family", "movement_pattern", "primary_muscles", "secondary_muscles", "equipment",
    "difficulty", "program_tags", "goals", "load_method", "one_rm_allowed", "rpe_range", "rep_range",
    "tempo", "rest_range", "technique", "common_errors", "regression", "progression", "client_tip",
    "coach_review_flags", "source_internal", "visual_id", "client_show_source", "active"
]


def parse_exercise_library_text(text: str) -> tuple[list[dict[str, str]], list[str]]:
    ...
    for idx, raw in enumerate(raw_parts, start=1):
        ...
        name = data.get("name_ru", "").strip()
        if not code or not name:
            warnings.append(f"Блок {idx}: отсутствует id или name_ru")
            continue
        records.append(data)
    if not records and not warnings:
        warnings.append("Упражнения не найдены. Используйте формат M17_EXERCISE_LIBRARY_V1.")
    return records, warnings


def repair_incomplete_back_block() -> dict:
    """Self-heal the standard BACK block without overwriting existing exercise cards or visuals.

    v0.6.14 introduced block navigation and exposed an old incomplete import where only one
    BACK exercise could be visible. This migration inserts only missing BACK-001…BACK-030.
    If the full base block exists but almost all rows are inactive, it re-activates only
    M17_BASE BACK rows. User-created/non-base rows are never touched.
    """
    repair_path = BASE_DIR / "M17_exercise_library_back_pulls_v1_import_repair.txt"
    if not repair_path.exists() or not COACH_IDS:
        return {"owners": 0, "created": 0, "reactivated": 0}
    text = repair_path.read_text(encoding="utf-8-sig")
    records, warnings = parse_exercise_library_text(text)
    if not records:
        raise RuntimeError(warnings[0] if warnings else "BACK repair file is empty")
    now = utc_now()
    created = 0
    reactivated = 0
    owners = 0
    with db_conn() as conn:
        for owner in sorted(COACH_IDS):
            ...
            total_row = conn.execute(
                "SELECT COUNT(*) AS n FROM exercise_library WHERE owner_telegram_id=? AND UPPER(exercise_code) LIKE 'BACK-%'",
                (str(owner),),
            ).fetchone()
            ...
            if total < len(records):
                for rec in records:
                    ...
                    exists = conn.execute(
                        "SELECT 1 AS ok FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?",
                        (str(owner), code),
                    ).fetchone()
                    if exists:
                        continue
                    values = {k: rec.get(k, "") for k in EXERCISE_LIBRARY_FIELDS}
                    values["client_show_source"] = 0
                    values["active"] = 1
                    cols = ["exercise_code", "owner_telegram_id"] + EXERCISE_LIBRARY_FIELDS + ["created_at", "updated_at"]
                    vals = [code, str(owner)] + [values[k] for k in EXERCISE_LIBRARY_FIELDS] + [now, now]
                    conn.execute(
                        "INSERT INTO exercise_library(" + ",".join(cols) + ") VALUES(" + ",".join(["?"] * len(cols)) + ")",
                        tuple(vals),
                    )
                    created += 1
            active_row = conn.execute(
                "SELECT COUNT(*) AS n FROM exercise_library WHERE owner_telegram_id=? AND UPPER(exercise_code) LIKE 'BACK-%' AND active=1",
                (str(owner),),
            ).fetchone()
            ...
            total_after = int(total_after_row["n"] or 0)
            if total_after >= 25 and active_count < 5:
                cur = conn.execute(
                    "UPDATE exercise_library SET active=1,updated_at=? WHERE owner_telegram_id=? AND UPPER(exercise_code) LIKE 'BACK-%' AND active=0 AND source_internal='M17_BASE'",
                    (now, str(owner)),
                )
                reactivated += int(cur.rowcount or 0)
        conn.commit()
    if created or reactivated:
        print(f"BACK block repaired: created={created}, reactivated={reactivated}")
    return {"owners": owners, "created": created, "reactivated": reactivated}


def exercise_row_dict(row) -> dict:
    out = dict(row)
    out["client_show_source"] = bool(out.get("client_show_source"))
    out["active"] = bool(out.get("active"))
    return out


VISUAL_WORKFLOW_STATUSES = {"NO_VISUAL", "PROMPT_READY", "CREATED", "REVIEW", "FIX_REQUIRED", "APPROVED"}


def visual_workflow_row(conn, owner: str, exercise_code: str):
    return conn.execute(
        "SELECT status,note,created_at,updated_at FROM exercise_visual_workflow WHERE owner_telegram_id=? AND exercise_code=?",
        (owner, exercise_code),
    ).fetchone()


def effective_visual_workflow(conn, owner: str, exercise_code: str, visual_count: Optional[int] = None) -> dict:
    row = visual_workflow_row(conn, owner, exercise_code)
    if row and str(row["status"] or "") in VISUAL_WORKFLOW_STATUSES:
        return {
            "status": row["status"],
            "note": row["note"] or "",
    ...
    if visual_count is None:
        visual_count = int(conn.execute(
            "SELECT COUNT(*) AS n FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=?",
            (owner, exercise_code),
        ).fetchone()["n"] or 0)
    return {"status": "CREATED" if int(visual_count or 0) > 0 else "NO_VISUAL", "note": "", "updated_at": None}


def set_visual_workflow(conn, owner: str, exercise_code: str, status: str, note: Optional[str] = None) -> dict:
    if status not in VISUAL_WORKFLOW_STATUSES:
        raise HTTPException(status_code=400, detail="Некорректный статус визуала")
    now = utc_now()
    existing = visual_workflow_row(conn, owner, exercise_code)
    clean_note = (note or "").strip() or None
    if existing:
        if note is None:
            clean_note = existing["note"]
        conn.execute(
            "UPDATE exercise_visual_workflow SET status=?,note=?,updated_at=? WHERE owner_telegram_id=? AND exercise_code=?",
            (status, clean_note, now, owner, exercise_code),
        )
    else:
        conn.execute(
            "INSERT INTO exercise_visual_workflow(owner_telegram_id,exercise_code,status,note,created_at,updated_at) VALUES(?,?,?,?,?,?)",
            (owner, exercise_code, status, clean_note, now, now),
        )
    return {"status": status, "note": clean_note or "", "updated_at": now}


def exercise_visual_meta(row) -> dict:
    return {
        "id": int(row["id"]),
        "sort_order": int(row["sort_order"]),
        "filename": row["filename"],
        "mime_type": row["mime_type"],
        "url": f"/exercise-visuals/{int(row['id'])}",
    }


def get_exercise_visuals(conn, owner: str, exercise_code: str) -> list[dict]:
    rows = conn.execute(
        "SELECT id,sort_order,filename,mime_type FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=? ORDER BY sort_order,id",
        (owner, exercise_code),
    ).fetchall()
    return [exercise_visual_meta(r) for r in rows]


@app.get("/exercise-visuals/{visual_id}")
def exercise_visual_image(visual_id: int):
    with db_conn() as conn:
        row = conn.execute("SELECT mime_type,image_data FROM exercise_visuals WHERE id=?", (visual_id,)).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Визуал не найден")
    try:
        raw = base64.b64decode(row["image_data"], validate=True)
    except Exception:
        raise HTTPException(status_code=500, detail="Визуал повреждён")
    return Response(content=raw, media_type=row["mime_type"], headers={"Cache-Control": "public, max-age=86400"})


@app.get("/coach/exercises/{exercise_code}/visuals")
def coach_exercise_visuals(exercise_code: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    with db_conn() as conn:
        exists = conn.execute("SELECT 1 AS ok FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code)).fetchone()
        if not exists:
            raise HTTPException(status_code=404, detail="Упражнение не найдено")
        return get_exercise_visuals(conn, owner, exercise_code)


@app.post("/coach/exercises/{exercise_code}/visuals")
def coach_exercise_visuals_upload(exercise_code: str, payload: ExerciseVisualUpload, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    ...
    for item in payload.images:
        ...
        if mime not in allowed_mime:
            raise HTTPException(status_code=400, detail=f"Неподдерживаемый формат {mime}. Используйте JPG, PNG или WEBP.")
        try:
            raw = base64.b64decode(item.data_base64, validate=True)
        except Exception:
            raise HTTPException(status_code=400, detail=f"Файл {item.filename}: некорректные данные изображения")
        if len(raw) > 5 * 1024 * 1024:
            raise HTTPException(status_code=400, detail=f"Файл {item.filename} больше 5 МБ")
        total_bytes += len(raw)
        decoded.append((item.filename, mime, base64.b64encode(raw).decode("ascii")))
    if total_bytes > 40 * 1024 * 1024:
        raise HTTPException(status_code=400, detail="Общий размер выбранных изображений больше 40 МБ. Уменьшите размер файлов и повторите загрузку.")
    now = utc_now()
    with db_conn() as conn:
        exists = conn.execute("SELECT 1 AS ok FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code)).fetchone()
        if not exists:
        ...
        if payload.replace_existing:
            conn.execute("DELETE FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code))
        existing_count = conn.execute("SELECT COUNT(*) AS n FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code)).fetchone()["n"]
        # v0.6.16: фиксированного лимита количества кадров больше нет.
        # При необходимости тренер добавляет изображения несколькими партиями.
        start_order = int(existing_count) + 1
        for offset, (filename, mime, data) in enumerate(decoded):
            conn.execute(
                "INSERT INTO exercise_visuals(owner_telegram_id,exercise_code,sort_order,filename,mime_type,image_data,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",
                (owner, exercise_code, start_order + offset, filename, mime, data, now, now),
            )
        # visual_id remains a compact compatibility marker for older clients.
        conn.execute("UPDATE exercise_library SET visual_id=?,updated_at=? WHERE owner_telegram_id=? AND exercise_code=?", (exercise_code, now, owner, exercise_code))
        workflow = set_visual_workflow(conn, owner, exercise_code, "CREATED")
        conn.commit()
        visuals = get_exercise_visuals(conn, owner, exercise_code)
    return {"ok": True, "count": len(visuals), "visuals": visuals, "visual_status": workflow["status"]}


@app.delete("/coach/exercises/{exercise_code}/visuals")
def coach_exercise_visuals_delete(exercise_code: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    with db_conn() as conn:
        conn.execute("DELETE FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code))
        conn.execute("UPDATE exercise_library SET visual_id=NULL,updated_at=? WHERE owner_telegram_id=? AND exercise_code=?", (utc_now(), owner, exercise_code))
        set_visual_workflow(conn, owner, exercise_code, "NO_VISUAL")
        conn.commit()
    return {"ok": True}


@app.get("/coach/visual-queue")
def coach_visual_queue(x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    with db_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM exercise_library WHERE owner_telegram_id=? AND active=1 ORDER BY exercise_code",
            (owner,),
        ).fetchall()
        result = []
        for r in rows:
            item = exercise_row_dict(r)
            count = int(conn.execute(
                "SELECT COUNT(*) AS n FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=?",
                (owner, r["exercise_code"]),
            ).fetchone()["n"] or 0)
            wf = effective_visual_workflow(conn, owner, r["exercise_code"], count)
            item["visual_count"] = count
            item["visual_status"] = wf["status"]
            item["visual_note"] = wf["note"]
            item["visual_status_updated_at"] = wf["updated_at"]
            result.append(item)
        return result


@app.put("/coach/exercises/{exercise_code}/visual-workflow")
def coach_exercise_visual_workflow(exercise_code: str, payload: ExerciseVisualWorkflowUpdate, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    with db_conn() as conn:
        ex = conn.execute("SELECT 1 AS ok FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code)).fetchone()
        if not ex:
            raise HTTPException(status_code=404, detail="Упражнение не найдено")
        result = set_visual_workflow(conn, owner, exercise_code, payload.status, payload.note)
        conn.commit()
        return {"ok": True, **result}


@app.post("/coach/visual-workflow/batch")
def coach_visual_workflow_batch(payload: ExerciseVisualWorkflowBatchUpdate, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    changed = 0
    skipped = 0
    with db_conn() as conn:
        for raw_code in payload.exercise_codes:
            code = str(raw_code or "").strip()
            if not code:
                skipped += 1
                continue
            ex = conn.execute("SELECT 1 AS ok FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, code)).fetchone()
            if not ex:
                skipped += 1
                continue
            current = effective_visual_workflow(conn, owner, code)["status"]
            if payload.only_if_status and current != payload.only_if_status:
                skipped += 1
                continue
            set_visual_workflow(conn, owner, code, payload.status)
            changed += 1
        conn.commit()
    return {"ok": True, "changed": changed, "skipped": skipped, "status": payload.status}


@app.get("/coach/exercises")
def coach_exercises(query: Optional[str] = None, family: Optional[str] = None, include_inactive: bool = False, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    with db_conn() as conn:
        where = ["owner_telegram_id=?"]
        params: list[Any] = [str(coach["id"])]
        if not include_inactive:
            where.append("active=1")
        if family:
            where.append("family=?"); params.append(family)
        if query:
            where.append("(LOWER(name_ru) LIKE ? OR LOWER(exercise_code) LIKE ? OR LOWER(primary_muscles) LIKE ? OR LOWER(equipment) LIKE ?)")
            q = f"%{query.lower()}%"; params.extend([q, q, q, q])
        rows = conn.execute("SELECT * FROM exercise_library WHERE " + " AND ".join(where) + " ORDER BY exercise_code", tuple(params)).fetchall()
        result = []
        for r in rows:
            item = exercise_row_dict(r)
            count = int(conn.execute("SELECT COUNT(*) AS n FROM exercise_visuals WHERE owner_telegram_id=? AND exercise_code=?", (str(coach["id"]), r["exercise_code"])).fetchone()["n"] or 0)
            item["visual_count"] = count
            wf = effective_visual_workflow(conn, str(coach["id"]), r["exercise_code"], count)
            item["visual_status"] = wf["status"]
            item["visual_note"] = wf["note"]
            result.append(item)
        return result
...
@app.get("/coach/exercises/{exercise_code}")
def coach_exercise(exercise_code: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    with db_conn() as conn:
        row = conn.execute("SELECT * FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (str(coach["id"]), exercise_code)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Упражнение не найдено")
        item = exercise_row_dict(row)
        item["visuals"] = get_exercise_visuals(conn, str(coach["id"]), exercise_code)
        item["visual_count"] = len(item["visuals"])
        wf = effective_visual_workflow(conn, str(coach["id"]), exercise_code, item["visual_count"])
        item["visual_status"] = wf["status"]
        item["visual_note"] = wf["note"]
        item["visual_status_updated_at"] = wf["updated_at"]
        return item
...
@app.post("/coach/exercises/import")
def coach_exercises_import(payload: ExerciseLibraryImport, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    records, warnings = parse_exercise_library_text(payload.text)
    if not records:
        raise HTTPException(status_code=400, detail=warnings[0] if warnings else "Упражнения не найдены")
    created = 0; updated = 0; skipped = 0
    owner = str(coach["id"]); now = utc_now()
    with db_conn() as conn:
        for rec in records:
            code = rec.get("id", "").strip()
            existing = conn.execute("SELECT id,visual_id FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, code)).fetchone()
            values = {k: rec.get(k, "") for k in EXERCISE_LIBRARY_FIELDS}
            values["client_show_source"] = 1 if str(rec.get("client_show_source", "false")).lower() in {"1","true","yes","да"} else 0
            values["active"] = 0 if str(rec.get("active", "true")).lower() in {"0","false","no","нет"} else 1
            if existing and not str(values.get("visual_id") or "").strip():
                # Импорт текстовой библиотеки не должен сбрасывать уже загруженный визуал.
                values["visual_id"] = existing["visual_id"] or ""
            if existing:
                if not payload.overwrite_existing:
                    skipped += 1; continue
                sets = ",".join([f"{k}=?" for k in EXERCISE_LIBRARY_FIELDS])
                params = [values[k] for k in EXERCISE_LIBRARY_FIELDS] + [now, owner, code]
                conn.execute(f"UPDATE exercise_library SET {sets},updated_at=? WHERE owner_telegram_id=? AND exercise_code=?", tuple(params))
                updated += 1
            else:
                cols = ["exercise_code","owner_telegram_id"] + EXERCISE_LIBRARY_FIELDS + ["created_at","updated_at"]
                vals = [code, owner] + [values[k] for k in EXERCISE_LIBRARY_FIELDS] + [now, now]
                conn.execute("INSERT INTO exercise_library(" + ",".join(cols) + ") VALUES(" + ",".join(["?"]*len(cols)) + ")", tuple(vals))
                created += 1
        link_report = link_existing_exercises_for_coach(conn, owner)
        conn.commit()
    return {"ok": True, "items_found": len(records), "created": created, "updated": updated, "skipped": skipped, "warnings": warnings, "link_report": link_report}


@app.post("/coach/exercises/link-existing")
def coach_link_existing_exercises(x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    with db_conn() as conn:
        report = link_existing_exercises_for_coach(conn, owner)
        conn.commit()
    return {"ok": True, **report}


@app.post("/coach/exercises/link-manual")
def coach_link_exercise_manual(payload: ExerciseManualLink, x_telegram_init_data: Optional[str] = Header(default=None)):
    """Сохраняет ручную связь одной строки тренировки с упражнением библиотеки.

    v0.6.9 deliberately does not depend on a second full re-scan. The endpoint
    validates ownership/source, replaces any previous AUTO/MANUAL links for the
    same line, and returns the saved link so the UI can confirm it immediately.
    """
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    code = str(payload.exercise_code or "").strip().upper()
    try:
        with db_conn() as conn:
            ex = conn.execute(
                "SELECT exercise_code,name_ru FROM exercise_library WHERE owner_telegram_id=? AND UPPER(exercise_code)=? AND active=1",
                (owner, code),
            ).fetchone()
            if not ex:
                raise HTTPException(status_code=404, detail=f"Упражнение {code} не найдено в активной библиотеке")

            if payload.source_type == "cycle":
                row = conn.execute(
                    """SELECT w.content_json FROM training_cycle_workouts w
                       JOIN training_cycles c ON c.id=w.cycle_id
                       WHERE w.id=? AND (c.owner_telegram_id=? OR c.owner_telegram_id IS NULL OR c.owner_telegram_id='')""",
                    (payload.source_id, owner),
                ).fetchone()
            else:
                row = conn.execute(
                    "SELECT content_json FROM workout_assignments WHERE id=?",
                    (payload.source_id,),
                ).fetchone()
            if not row:
                raise HTTPException(status_code=404, detail="Исходная тренировка не найдена или недоступна этому тренеру")

            raw_content = row["content_json"]
            if isinstance(raw_content, dict):
                content = raw_content
            else:
                content = safe_json(raw_content, {})
            text = str((content or {}).get("text", "") or "")
            # Assignment text is normalized during the bulk matcher. Use the same
            # normalization here so the line numbers remain consistent.
            match_text = normalize_workout_text_ru(text) if payload.source_type == "assignment" else text
            line_map = dict(_workout_match_lines(match_text))
            source_line = line_map.get(int(payload.line_no))
            if not source_line:
                raise HTTPException(status_code=409, detail="Строка тренировки изменилась. Нажмите «Проверить и связать» ещё раз.")

            conn.execute(
                "DELETE FROM workout_exercise_links WHERE owner_telegram_id=? AND source_type=? AND source_id=? AND line_no=?",
                (owner, payload.source_type, int(payload.source_id), int(payload.line_no)),
            )
            now = utc_now()
            conn.execute(
                "INSERT INTO workout_exercise_links(owner_telegram_id,source_type,source_id,line_no,source_line,exercise_code,match_type,confidence,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                (owner, payload.source_type, int(payload.source_id), int(payload.line_no), source_line, ex["exercise_code"], "MANUAL", 1.0, now, now),
            )
            conn.commit()
            return {
                "ok": True,
        ...
        print(f"Manual exercise link failed: {type(exc).__name__}: {exc}")
        raise HTTPException(status_code=409, detail=f"Не удалось сохранить связь: {type(exc).__name__}. Обновите страницу и попробуйте снова.")


@app.put("/coach/exercises/{exercise_code}")
def coach_exercise_edit(exercise_code: str, payload: ExerciseLibraryEdit, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    owner = str(coach["id"])
    ...
    if not data:
        return {"ok": True}
    with db_conn() as conn:
        row = conn.execute("SELECT * FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Упражнение не найдено")
        sets=[]; params=[]
        for key, value in data.items():
            if key not in EXERCISE_LIBRARY_FIELDS:
                continue
            if key in {"client_show_source","active"}:
                value = 1 if value else 0
            sets.append(f"{key}=?"); params.append(value)
        sets.append("updated_at=?"); params.append(utc_now()); params.extend([owner, exercise_code])
        conn.execute("UPDATE exercise_library SET " + ",".join(sets) + " WHERE owner_telegram_id=? AND exercise_code=?", tuple(params))
        conn.commit()
        row = conn.execute("SELECT * FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, exercise_code)).fetchone()
        return exercise_row_dict(row)


@app.delete("/coach/exercises/{exercise_code}")
def coach_exercise_archive(exercise_code: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach = coach_user(x_telegram_init_data)
    with db_conn() as conn:
        cur = conn.execute("UPDATE exercise_library SET active=0,updated_at=? WHERE owner_telegram_id=? AND exercise_code=?", (utc_now(), str(coach["id"]), exercise_code))
        conn.commit()
        if cur.rowcount == 0:
            raise HTTPException(status_code=404, detail="Упражнение не найдено")
    return {"ok": True}
...
def coach_library_add_workout(cycle_id: int, payload: TrainingCycleWorkoutCreate, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        link_workout_text(conn, str(coach["id"]), "cycle", int(wid), normalize_workout_text_ru(payload.text), preserve_manual=False)
...
def coach_library_edit_workout(cycle_id: int, workout_id: int, payload: TrainingCycleWorkoutEdit, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        conn.execute("DELETE FROM workout_exercise_links WHERE source_type='cycle' AND source_id=?", (workout_id,))
        link_workout_text(conn, str(coach["id"]), "cycle", int(workout_id), text, preserve_manual=False)
        conn.commit()
    return {"ok": True}
...
def coach_library_import_text(payload: TrainingBulkTextImport, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        for idx, text in enumerate(parts):
            ...
            wid = add_cycle_workout(conn, cid, item)
            link_workout_text(conn, str(coach["id"]), "cycle", int(wid), normalize_workout_text_ru(text), preserve_manual=False)
...
def coach_library_import_telegram_json(payload: TrainingTelegramImport, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    first_date = dated[0]["date"] if dated else app_today()
    ...
    with db_conn() as conn:
        ...
        for idx, msg in enumerate(found):
            ...
            wid = add_cycle_workout(conn, cid, item)
            link_workout_text(conn, str(coach["id"]), "cycle", int(wid), normalize_workout_text_ru(msg["text"]), preserve_manual=False)
...
async def coach_library_assign_cycle(cycle_id: int, client_id: str, payload: TrainingCycleAssign, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        audience = str(cycle["audience"] or "all")
        client_sex = str(client["sex"] or "not_specified")
        if audience in {"female", "male"} and client_sex != audience:
            label = "женщинам" if audience == "female" else "мужчинам"
            raise HTTPException(status_code=400, detail=f"Этот цикл предназначен {label}. Проверьте пол клиента или выберите другой цикл.")
        ...
        program_id, count = assign_cycle_to_client(conn, cycle, workouts, client, start, payload.replace_future, payload.activate_program, str(coach["id"]))
        ...
        conn.commit()
    await notify_client(client_id, {"type": "program_updated"})
    notification = await send_client_message(client_id, f"Тренер назначил программу «{cycle['title']}». Откройте M17, чтобы приступить к тренировкам.")
    return {"ok": True, "program_id": program_id, "workouts_assigned": count, "notification": notification}
