...
app = FastAPI(title="M17 Training System", version="0.5.1")
...
def build_plan(req: PlanRequest) -> PlanPreview:
    ...
    if req.goal in {Goal.ironman, Goal.half_ironman, Goal.olympic, Goal.sprint}:
        ...
        blocks = [
            TrainingBlock(name="Easy Run", sessions=max(1, runs - 2), priority="primary", note="Аэробная база / Z2"),
            TrainingBlock(name="Quality Run", sessions=1, priority="primary", note="Темп, порог или интервалы по этапу"),
            TrainingBlock(name="Long Run", sessions=1, priority="primary", note="Постепенное развитие длительной выносливости"),
            TrainingBlock(name="Strength", sessions=2 if req.days_per_week >= 5 else 1, priority="support", note="Ноги, ягодичные, стопа, корпус"),
            TrainingBlock(name="Mobility / Prehab", sessions=1, priority="support", note="Стопа, голеностоп, таз, задняя цепь"),
        ]
        ...
        blocks = [
            TrainingBlock(name="Strength", sessions=min(req.days_per_week, 4), priority="primary", note="Базовые движения + индивидуальные слабые звенья"),
            TrainingBlock(name="Zone 2", sessions=1, priority="support", note="Поддержание аэробной формы"),
            TrainingBlock(name="Mobility / Prehab", sessions=1, priority="support", note="Рабочая амплитуда и восстановление"),
        ]
    else:
        strength = 3 if req.days_per_week == 3 else min(req.days_per_week, 5)
        blocks = [
            TrainingBlock(name="Hypertrophy Strength", sessions=strength, priority="primary", note="6–15 повторов, прогрессивная перегрузка, контроль RIR/RPE"),
...
def row_to_profile(row) -> SavedClientProfile:
    ...
    return SavedClientProfile(
        ...
        goal_details=safe_json(d.get("goal_details_json"), {}), level=d["level"], days_per_week=d["days_per_week"],
...
GOAL_LABELS = {
    ...
    "sprint": "Спринт-триатлон", "running": "Бег", "crossfit": "CrossFit",
...
def workout_text(kind: str, level: str, week: int, focus: list[str]) -> str:
    ...
    if kind == "strength_glute":
        squat = "Goblet squat 3×8 @ RPE 6–7" if novice else "Back squat 4×6 @ RPE 7"
        hip = "Hip thrust 3×10 @ RPE 7" if novice else "Hip thrust 4×8 @ RPE 7–8"
        split = "Split squat 3×8/8" if novice else "Bulgarian split squat 3×8/8 @ RPE 7"
        items = ["Разминка: 8–10 мин + таз/голеностоп.", squat, hip, split, "Romanian deadlift 3×8 @ RPE 7"]
        if glute:
            items.append("Отведение бедра / band abduction 3×12–15")
        if core:
            items.append("Dead bug 3×8/8 + side plank 3×30 сек/сторона")
    ...
    if kind == "crossfit_intro":
        return "Разминка 10–12 мин.\nТехника: присед + hinge + жим, 3 спокойных круга.\nСила: Goblet squat 4×6 @ RPE 6–7.\nSkill: 10 мин — ring row / scap pull-up / double-under practice по уровню.\nConditioning 12 мин AMRAP: 8 cal Row, 8 DB deadlift, 8 box step-up. RPE 6–7, без отказа.\nMobility 8 мин."
    if kind == "crossfit_advanced":
        return "Разминка 12–15 мин.\nСила: Back squat 5×3 @ 75–80%, RPE ≤8.\nSkill: EMOM 10 — 1) гимнастический навык 20–30 сек, 2) 40–50 double-unders.\nMetcon: 5 раундов, 12 cal Ski + 8 умеренных DB snatch + 6 burpee box jump-over; отдых 1:00. Цель — ровная плотность, не отказ.\nAccessory: posterior chain + rotator cuff."
    if kind == "zone2":
        return "30–45 мин Zone 2 на Bike / Row / Ski / быстрая ходьба. RPE 3–4. Дышать контролируемо. После — 10 мин мобилити."
    if kind == "swim":
    ...
    if kind == "bike":
        return "Bike 60–75 мин.\n15 мин легко.\n3×8 мин tempo / RPE 6, между 4 мин легко.\nОстаток времени Z2. Каденс комфортный, без силового продавливания."
    if kind == "tri_run":
        return "Бег 35–50 мин Z2 / RPE 3–4. В конце 4×20 сек лёгких ускорений с полным восстановлением."
    if kind == "brick":
        return "Bike 45–60 мин Z2, последние 10 мин RPE 5. Быстрый переход.\nRun 10–20 мин легко, задача — техника и адаптация ног, не скорость."
    ...
    if kind == "hypertrophy_lower":
        return "Присед / leg press 4×8 @ RPE 7–8.\nHip thrust 4×8–10.\nRomanian deadlift 3×8–10.\nSplit squat 3×10/10.\nСгибание голени 3×12.\nИкры 3×12–15."
...
def weekly_kinds(row) -> list[str]:
    ...
    days = int(row["days_per_week"])
    level = row["level"]
    focus = set(safe_json(row["body_focus_json"], []))

    if primary in {"ironman", "half_ironman", "olympic", "sprint"}:
        seq = ["swim", "bike", "tri_run", "strength_full", "swim", "brick", "bike"]
    elif primary == "running" or "running" in all_goals:
        seq = ["strength_glute" if ("glutes" in focus or "fat_loss" in all_goals or "muscle_gain" in all_goals) else "strength_full",
               "run_easy", "strength_glute" if "glutes" in focus else "strength_full", "run_quality", "zone2", "run_long", "zone2"]
    elif primary == "crossfit":
        cf = "crossfit_intro" if level == "beginner" else "crossfit_advanced"
        seq = [cf, "zone2", cf, "strength_full", cf, "zone2", cf]
    ...
    # Choose training days spread through the week, preserving long/endurance work late in week.
    indexes = {2: [1, 5], 3: [0, 2, 5], 4: [0, 1, 3, 5], 5: [0, 1, 2, 4, 5], 6: [0, 1, 2, 3, 4, 5], 7: list(range(7))}[days]
...
def generate_intro_workouts(conn, row, program_id: Optional[int], start: date, weeks=4):
    ...
    day_indexes = {2: [1, 5], 3: [0, 2, 5], 4: [0, 1, 3, 5], 5: [0, 1, 2, 4, 5], 6: [0, 1, 2, 3, 4, 5], 7: list(range(7))}[int(row["days_per_week"])]
    focus = safe_json(row["body_focus_json"], [])
    level = row["level"]
    labels = {
        "run_easy": "Лёгкий бег", "run_quality": "Бег · интервалы / порог", "run_long": "Длительный бег",
        "strength_glute": "Сила · ягодичные + core", "strength_full": "Силовая · Full Body",
        "crossfit_intro": "CrossFit · техника + кондиция", "crossfit_advanced": "CrossFit · strength + skill + metcon",
        "zone2": "Zone 2 / восстановление", "swim": "Плавание", "bike": "Велосипед", "tri_run": "Бег · триатлон",
        "brick": "Brick · Bike → Run", "hypertrophy_upper": "Гипертрофия · верх", "hypertrophy_lower": "Гипертрофия · низ",
    }
...
def health():
    return {
        "status": "ok", "telegram_configured": bool(TOKEN and PUBLIC_URL), "version": "0.5.1",
        "storage": "postgres" if USE_POSTGRES else "sqlite", "coach_configured": bool(COACH_IDS),
        "features": ["multi_goal", "subscriptions", "daily_workouts", "mandatory_feedback", "live_edit"],
    }
...
def save_profile(profile: ClientProfile, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        conn.execute(
            """
            INSERT INTO clients (telegram_user_id,telegram_username,name,sex,age,height_cm,weight_kg,experience_years,primary_goal,level,days_per_week,hours_per_week,weeks_to_event,equipment_json,limitations,notes,status,coach_notes,created_at,updated_at,client_type,goals_json,body_focus_json,goal_details_json)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ON CONFLICT(telegram_user_id) DO UPDATE SET telegram_username=excluded.telegram_username,name=excluded.name,sex=excluded.sex,age=excluded.age,height_cm=excluded.height_cm,weight_kg=excluded.weight_kg,experience_years=excluded.experience_years,primary_goal=excluded.primary_goal,level=excluded.level,days_per_week=excluded.days_per_week,hours_per_week=excluded.hours_per_week,weeks_to_event=excluded.weeks_to_event,equipment_json=excluded.equipment_json,limitations=excluded.limitations,notes=excluded.notes,client_type=excluded.client_type,goals_json=excluded.goals_json,body_focus_json=excluded.body_focus_json,goal_details_json=excluded.goal_details_json,updated_at=excluded.updated_at
            """,
            (
...
def client_home(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        today_workout = conn.execute("SELECT * FROM workout_assignments WHERE client_id=? AND workout_date=? ORDER BY id LIMIT 1", (uid, date.today().isoformat())).fetchone()
        upcoming = conn.execute("SELECT * FROM workout_assignments WHERE client_id=? AND workout_date>=? ORDER BY workout_date,id LIMIT 14", (uid, date.today().isoformat())).fetchall()
    def wdict(w):
        if not w:
            return None
        d = dict(w); d["content"] = safe_json(d.pop("content_json", "{}"), {}); return d
    return {
        ...
        "today_workout": wdict(today_workout) if access["allowed"] else None,
        "upcoming": [wdict(x) for x in upcoming] if access["allowed"] else [],
    }
...
def client_workouts(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        rows = conn.execute("SELECT * FROM workout_assignments WHERE client_id=? ORDER BY workout_date DESC,id DESC LIMIT 60", (uid,)).fetchall()
    out=[]
    for r in rows:
        d=dict(r); d["content"]=safe_json(d.pop("content_json","{}"),{}); out.append(d)
    return out
...
def coach_dashboard(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    return {"clients": len(rows), "new_clients": new_count, "active_clients": active_count, "alerts_count": len(alerts), "alerts": alerts[:10], "payments_due_7": pay["due_7"], "payments_expired": pay["expired"]}
    ...
    with db_conn() as conn:
        ...
        for r in rows:
            ...
            result.append({
                ...
                "alert": client_alert(conn, r["telegram_user_id"]), "access": access,
...
def coach_client_detail(client_id: str, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        workouts = conn.execute("SELECT * FROM workout_assignments WHERE client_id=? AND workout_date>=? ORDER BY workout_date,id LIMIT 30", (client_id, date.today().isoformat())).fetchall()
        sub = subscription_row(conn, client_id)
        alert = client_alert(conn, client_id)
    ws=[]
    for w in workouts:
        d=dict(w); d["content"]=safe_json(d.pop("content_json","{}"),{}); ws.append(d)
    return {
        "profile": row_to_profile(row).model_dump(), "alert": alert, "subscription": dict(sub) if sub else None,
...
async def telegram_webhook(request: Request, x_telegram_bot_api_secret_token: Optional[str] = Header(default=None)):
    ...
    if chat_id and text.startswith("/id"):
        ...
        await telegram_api("sendMessage", {"chat_id": chat_id, "text": f"M17 · {role}. Откройте приложение.", "reply_markup": {"inline_keyboard": [[{"text": "Открыть M17", "web_app": {"url": PUBLIC_URL}}]]}})
    return {"ok": True}
