...
def create_training_cycle(conn, payload: TrainingCycleCreate) -> int:
    now = utc_now()
    cycle_id = insert_and_get_id(
        conn,
        "INSERT INTO training_cycles(title,sport,level,audience,duration_weeks,sessions_per_week,phase,status,source,notes,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
        (payload.title, payload.sport, payload.level, payload.audience, payload.duration_weeks, payload.sessions_per_week, payload.phase, "ACTIVE", payload.source, payload.notes, now, now),
    )
    return cycle_id
...
def training_cycle_dict(conn, row, include_workouts=True) -> dict:
    out = dict(row)
...
def coach_library_cycles(sport: Optional[str] = None, level: Optional[str] = None, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        for r in rows:
            ...
            result.append(training_cycle_dict(conn, r, include_workouts=False) | {"workout_count": int(count)})
    return result
...
def coach_library_create_cycle(payload: TrainingCycleCreate, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach_user(x_telegram_init_data)
    with db_conn() as conn:
        cid = create_training_cycle(conn, payload)
        conn.commit()
        row = conn.execute("SELECT * FROM training_cycles WHERE id=?", (cid,)).fetchone()
        return training_cycle_dict(conn, row, include_workouts=True)
...
def coach_library_add_workout(cycle_id: int, payload: TrainingCycleWorkoutCreate, x_telegram_init_data: Optional[str] = Header(default=None)):
    coach_user(x_telegram_init_data)
    with db_conn() as conn:
        if not conn.execute("SELECT 1 AS ok FROM training_cycles WHERE id=?", (cycle_id,)).fetchone():
            raise HTTPException(status_code=404, detail="Цикл не найден")
...
def coach_library_import_text(payload: TrainingBulkTextImport, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        cid = create_training_cycle(conn, cycle)
        days = [0, 2, 4, 1, 3, 5, 6]
