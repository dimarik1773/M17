...
app = FastAPI(title="M17 Training System", version="0.4")
...
@app.get("/health")
def health():
    return {"status": "ok", "telegram_configured": bool(TOKEN and PUBLIC_URL), "version": "0.4", "client_db": True, "storage": "postgres" if USE_POSTGRES else "sqlite", "coach_configured": bool(COACH_IDS)}
...
def coach_create_program(client_id: str, payload: ProgramCreate, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        if USE_POSTGRES:
            ...
            program_id = inserted["id"] if inserted else None
        else:
