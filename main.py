    ...
    with db_conn() as conn:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS workout_exercise_links (
                ...
                source_type TEXT NOT NULL, source_id INTEGER NOT NULL, line_no INTEGER NOT NULL,
...
def coach_link_exercise_manual(payload: ExerciseManualLink, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    owner = str(coach["id"])
    with db_conn() as conn:
        ex = conn.execute("SELECT 1 AS ok FROM exercise_library WHERE owner_telegram_id=? AND exercise_code=?", (owner, payload.exercise_code)).fetchone()
        if not ex:
            raise HTTPException(status_code=404, detail="Код упражнения не найден в библиотеке")
        if payload.source_type == "cycle":
            row = conn.execute("SELECT content_json FROM training_cycle_workouts WHERE id=?", (payload.source_id,)).fetchone()
        else:
            row = conn.execute("SELECT content_json FROM workout_assignments WHERE id=?", (payload.source_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Исходная тренировка не найдена")
        text = safe_json(row["content_json"], {}).get("text", "")
        line_map = dict(_workout_match_lines(text))
        source_line = line_map.get(payload.line_no)
        if not source_line:
            raise HTTPException(status_code=400, detail="Строка тренировки изменилась. Запустите проверку привязок ещё раз.")
        conn.execute("DELETE FROM workout_exercise_links WHERE owner_telegram_id=? AND source_type=? AND source_id=? AND line_no=?", (owner, payload.source_type, payload.source_id, payload.line_no))
        now = utc_now()
        conn.execute(
            "INSERT INTO workout_exercise_links(owner_telegram_id,source_type,source_id,line_no,source_line,exercise_code,match_type,confidence,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
            (owner, payload.source_type, payload.source_id, payload.line_no, source_line, payload.exercise_code, "MANUAL", 1.0, now, now),
        )
        conn.commit()
    return {"ok": True}
