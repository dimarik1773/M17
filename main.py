...
def health():
    return {
        ...
        "features": ["multi_goal", "training_days", "triathlon_distances", "running_distances", "trail_running", "sequential_workouts", "vacation_mode", "subscriptions", "mandatory_feedback", "live_edit", "athlete_metrics", "training_library", "telegram_json_import", "bulk_text_import", "cycle_assignment", "1rm", "ftp", "thresholds", "two_week_testing", "figure_single_branch", "client_targeted_notifications", "exercise_library", "exercise_library_import"],
    }
...
def coach_exercises(query: Optional[str] = None, family: Optional[str] = None, include_inactive: bool = False, x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        return [exercise_row_dict(r) for r in rows]
    ...
    with db_conn() as conn:
        for rec in records:
            ...
            if existing:
                ...
                created += 1
        conn.commit()
    return {"ok": True, "items_found": len(records), "created": created, "updated": updated, "skipped": skipped, "warnings": warnings}
