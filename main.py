...
app = FastAPI(title="M17 Training System", version="0.5.2")
...
def health():
    return {
        "status": "ok", "telegram_configured": bool(TOKEN and PUBLIC_URL), "version": "0.5.2",
...
def client_home(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    if current and access["allowed"]:
        ...
        if cdate and cdate <= date.today():
            ...
            next_locked = {"workout_date": current["workout_date"], "title": current["title"]}
    return {
        ...
        "history": [client_workout_view(x, include_content=False) for x in history],
        "vacation": vac,
    }
...
def client_workouts(x_telegram_init_data: Optional[str] = Header(default=None)):
    ...
    with db_conn() as conn:
        ...
        current = earliest_open_workout(conn, uid)
        history = conn.execute("SELECT * FROM workout_assignments WHERE client_id=? AND status IN ('COMPLETED','PARTIAL','NOT_DONE') ORDER BY workout_date DESC,id DESC LIMIT 60", (uid,)).fetchall()
    current_view = None
    if current:
        cdate = parse_iso_date(current["workout_date"])
        current_view = client_workout_view(current, include_content=(current["status"] == "OPENED" and cdate and cdate <= date.today()))
        current_view["locked_by_date"] = bool(cdate and cdate > date.today())
    return {"current": current_view, "history": [client_workout_view(x, include_content=False) for x in history]}
